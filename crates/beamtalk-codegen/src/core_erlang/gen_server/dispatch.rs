...
use beamtalk_core::ast::{Block, Expression, MethodKind, Module};
...
impl CoreErlangGenerator {
    ...
    pub(in crate::core_erlang) fn generate_method_table(
        &self,
        module: &Module,
    ...
    /// ```
    #[allow(clippy::unnecessary_wraps)] // uniform Result<Document> codegen interface
    pub(in crate::core_erlang) fn generate_class_name_function(
        &self,
        _module: &Module,
    ) -> Result<Document<'static>> {
        let doc = docvec![
                ...
                docvec![
                    ...
                    nest(
                        ...
                        docvec![
                            ...
                            line(),
                            "%% BT-3199: Selector/ClassName are threaded through so a",
                            line(),
                            "%% crashing extension body is caught and converted to a",
                            line(),
                            "%% structured error naming what was sent, instead of",
                            line(),
                            "%% re-raising a bare Erlang exception.",
                            line(),
                            "call 'beamtalk_dispatch':'invoke_extension'(ExtFun, OtherSelector, ",
                            leaf::atom(class_name.to_string()),
                            ", Args, Self, State)",
                        ]
                    ),
