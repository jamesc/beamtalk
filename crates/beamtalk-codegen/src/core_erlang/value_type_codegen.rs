...
enum VtBodyExprKind {
    ...
    /// Non-last `on:do:`/`ensure:` with local mutations (BT-3177) and/or a
    /// value-type `self.field := ...` write (BT-3486).
...
impl CoreErlangGenerator {
    ...
    fn generate_delegating_new(&mut self, class: &ClassDefinition) -> Result<Document<'static>> {
        ...
        let prim_name = new_method.body.iter().find_map(|stmt| {
            if let beamtalk_core::ast::Expression::Primitive { name, .. } = &stmt.expression {
                Some(name.clone())
            } else {
                None
            }
    ...
    pub(in crate::core_erlang) fn emit_vt_last_expr(
        &mut self,
        expr: &Expression,
        index: usize,
        has_nlr: bool,
    ...
    pub(in crate::core_erlang) fn generate_vt_empty_body(
        &self,
        has_nlr: bool,
    ) -> Vec<Document<'static>> {
        if has_nlr {
    ...
    pub(in crate::core_erlang) fn generate_vt_body_exprs(
        &mut self,
        body: &[&Expression],
        has_nlr: bool,
    ...
    fn emit_vt_body_expr(
        ...
        match kind {
            ...
            VtBodyExprKind::ExceptionConstructWithThreading => {
                ...
                // mutations. BT-3486 adds the value-type `self.field := ...`
                // case to the same non-last path. Last position is not yet
    ...
    pub(in crate::core_erlang) fn native_delegate_body_doc(
        backing_module: &str,
        class_name: &str,
    ...
    pub(in crate::core_erlang) fn is_while_with_vt_local_threading(
        &self,
        expr: &Expression,
    ) -> bool {
        if !self.in_class_method() && !matches!(self.context, CodeGenContext::ValueType) {
    ...
    pub(in crate::core_erlang) fn generate_vt_while_open(
        &mut self,
        expr: &Expression,
    ) -> Result<Document<'static>> {
    ...
    pub(in crate::core_erlang) fn emit_vt_threaded_local_assignment(
        &mut self,
        var_name: &str,
        value: &Expression,
        body_parts: &mut Vec<Document<'static>>,
    ...
    pub(in crate::core_erlang) fn is_counted_loop_with_vt_local_threading(
        &self,
        expr: &Expression,
    ) -> bool {
        if !self.in_class_method() && !matches!(self.context, CodeGenContext::ValueType) {
    ...
    pub(in crate::core_erlang) fn generate_vt_counted_loop_open(
        &mut self,
        expr: &Expression,
    ) -> Result<Document<'static>> {
    ...
    /// consistent.
    pub(in crate::core_erlang) fn is_foldl_list_op_with_vt_local_threading(
        &self,
        expr: &Expression,
    ) -> bool {
        if !self.in_class_method() && !matches!(self.context, CodeGenContext::ValueType) {
    ...
    pub(in crate::core_erlang) fn generate_vt_foldl_list_op_open(
        &mut self,
        expr: &Expression,
    ) -> Result<Document<'static>> {
    ...
    pub(in crate::core_erlang) fn generate_value_type_do_open(
        &mut self,
        expr: &Expression,
    ) -> Result<Document<'static>> {
    ...
    pub(in crate::core_erlang) fn is_conditional_with_vt_local_threading(
        &self,
        expr: &Expression,
    ) -> bool {
        if !self.in_class_method() && !matches!(self.context, CodeGenContext::ValueType) {
    ...
    pub(in crate::core_erlang) fn generate_vt_conditional_open(
        &mut self,
        expr: &Expression,
    ) -> Result<Document<'static>> {
        let (receiver, selector_name, arguments) = match expr {
    ...
    /// BT-3486: the protected block and the handler/cleanup block of an
    ...
    /// BT-3486: `true` if `expr` is an `on:do:`/`ensure:` whose protected or
    ...
    pub(in crate::core_erlang) fn emit_vt_conditional_assign_rhs(
        &mut self,
        var_name: &str,
        expr: &Expression,
        body_parts: &mut Vec<Document<'static>>,
