...
/// ancestor `gen_server`). When the user writes `self superclass` inside a
/// class method, the self-send must compile to a direct module call — the
/// BT-2007 runtime helper would raise `does_not_understand` for them.
///
...
/// only `superclass` and `class_name` are plausibly reachable via Beamtalk
/// source (`method_table` is a codegen-internal metadata accessor without a
/// stable user-level API; `__beamtalk_meta`, `register_class`, `has_method`
/// are similarly internal). `methods` was initially in this list by mistake
/// — no class module exports `methods/0`, so including it would have
/// compiled `self methods` to a non-existent direct call (would crash with
/// `undef`). Any auto-export added later that is reachable via plain
/// self-send needs to be added here. Unknown selectors take the
/// inherited-dispatch path, which raises structured DNU on miss — strictly
/// better than the pre-BT-2007 fallthrough (direct call → runtime `undef`).
pub(super) fn is_class_auto_export_selector(selector_atom: &str, arity: usize) -> bool {
    arity == 0 && matches!(selector_atom, "superclass" | "class_name")
}
