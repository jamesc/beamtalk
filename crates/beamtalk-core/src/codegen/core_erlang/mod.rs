    ...
    fn generate_primitive(
        ...
        if !is_quoted && self.in_class_method() {
            match name {
                ...
                // BT-3071: Actor.bt's `class sealed new` / `class sealed new:`
                // lift the actor instantiation-error stubs into real,
                // documented class-method declarations. The compiled body
                // raises the identical `instantiation_error` the per-actor-
                // module `new/0` / `new/1` stubs raise (still generated by
                // `generate_actor_new_error_method` /
                // `generate_actor_new_with_args_error_method` — required by
                // `beamtalk_class_instantiation:handle_new_compiled/4`'s
                // `erlang:apply(Module, new, Args)` fast path, which bypasses
                // class-method dispatch entirely). This intrinsic body is
                // unreachable via normal dispatch today (`new`/`new:` are
                // short-circuited in `beamtalk_class_dispatch:class_send/3`
                // before reaching the class-method chain) but stays correct
                // in case that changes (ADR 0032 Phase 4+).
                "actorNewError" => {
                    return Ok(Self::instantiation_error_expr(
                        "Actor",
                        "new",
                        "Use spawn instead",
                    ));
