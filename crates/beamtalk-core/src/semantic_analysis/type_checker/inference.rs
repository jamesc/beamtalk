...
impl TypeChecker {
        ...
        for class in &module.classes {
            ...
            for method in &class.methods {
                ...
                method_env.set_local(
                    "self",
                    type_resolver::receiver_type_for_class(&class.name.name, hierarchy),
                );
    ...
    pub(super) fn set_param_types(
        ...
        for param in parameters {
            let ty = match &param.type_annotation {
                Some(ann) => {
                    let (ty, deps) = type_resolver::resolve_type_annotation_with_alias_deps(
                        ann,
                        &subst,
                        protocol_registry,
                        alias_registry,
    ...
    pub(super) fn infer_literal_index_tuple_at(
        class_name: &str,
        selector_name: &str,
    ...
    pub(super) fn extract_ffi_function_info(
        selector: &MessageSelector,
        arguments: &[Expression],
    ) -> Option<(String, u8)> {
    ...
    pub(super) fn substitute_ffi_return_type(
    ...
    pub(super) fn is_self_receiver(expr: &Expression) -> bool {
    ...
    pub(super) fn refine_result_narrowing(
    ...
    pub(super) fn resolve_narrowing_variable_type(
        var_key: &EnvKey,
        env: &TypeEnv,
        hierarchy: &ClassHierarchy,
        alias_registry: Option<&AliasRegistry>,
    ...
    pub(super) fn infer_method_local_params(
