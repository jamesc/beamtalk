...
//! `Collection>>detect:ifNone:` must widen its result type through
...
//!
//! `detect:ifNone:` (and its `List`/`Stream`/
//! `SupervisionTree` overrides) declares the `ifNone:` block parameter and
//! return type generically (`Block(T)` / `E | T`), not `Block(Object)` /
//! `E | Object` unconditionally — `Object` is not `Nil`, so after an `isNil
//! ifTrue: [^...]` early return, the type checker's nil-narrowing (which
//! only removes a `Nil` arm from a union) would otherwise leave the value
//! typed `E | Object` — unchanged — and sending it any `E`-only message
//! would raise a spurious "Object does not understand ...
