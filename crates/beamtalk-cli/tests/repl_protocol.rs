...
fn test_cases_dir() -> PathBuf {
    workspace_root().join("tests/repl-protocol/cases")
}
...
fn beam_eval_cmd(
    ...
    if cover {
        ...
        let prelude = repl_startup::startup_prelude(0, None, "info");
        format!(
            "cover:start(), \
...
impl ReplClient {
    ...
    ///
    /// Uses the `tests/repl-protocol/fixtures/incremental_project` fixture which has a
    /// `beamtalk.toml` and `src/` with `.bt` files.
    fn sync_project(&mut self) -> Result<String, String> {
        let root = workspace_root();
        let project_path = root.join("tests/repl-protocol/fixtures/incremental_project");
...
fn e2e_language_tests() {
    ...
    if !cases_dir.exists() {
        ...
        eprintln!("Skipping E2E tests. Create test cases in tests/repl-protocol/cases/*.btscript");
        return;
    }
...
// @load-error tests/repl-protocol/fixtures/sealed.bt => Cannot subclass sealed class
...
";
        ...
        assert_eq!(
            ...
            "tests/repl-protocol/fixtures/bad_class.bt"
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            "tests/repl-protocol/fixtures/sealed.bt"
        );
        assert_eq!(
    ...
    fn test_parse_load_error_not_confused_with_load() {
        let content = r"
// @load tests/repl-protocol/fixtures/counter.bt
// @load-error tests/repl-protocol/fixtures/bad.bt => some error
...
";
        ...
        assert_eq!(
            parsed.load_error_cases[0].path,
            "tests/repl-protocol/fixtures/bad.bt"
        );
        assert!(parsed.warnings.is_empty());
    }
    ...
    fn test_parse_load_error_malformed_warns() {
        // Missing => separator
        let content = "// @load-error tests/repl-protocol/fixtures/bad.bt\n";
        let parsed = parse_test_file(content);
