...
pub fn build(path: &str, options: &beamtalk_core::CompilerOptions) -> Result<()> {
    ...
    if let Some(ref pkg) = pkg_manifest {
        generate_package_outputs(&build_dir, &project_root, pkg, &module_names)?;
    }
...
fn generate_package_outputs(
    ...
    let app_callback_module =
        if let Some(ref app_config) = manifest::find_application_config(project_root)? {
            ...
            generate_otp_app_callback(build_dir, pkg, &app_config.supervisor, &cb_module_name)?;
            info!(
...
fn generate_otp_app_callback(
    ...
    let sup_module = format!(
        "bt@{}@{}",
        pkg.name,
        beamtalk_core::codegen::core_erlang::to_module_name(supervisor_class)
    );
    ...
    let src = format!(
        "%% Copyright 2026 James Casey\n\
         %% SPDX-License-Identifier: Apache-2.0\n\
         %%\n\
         %% Generated OTP application callback for package '{pkg_name}'.\n\
         ...
         \n\
         start(_Type, _Args) ->\n\
             case '{sup_mod}':'start_link'() of\n\
                 {{ok, Pid}} = Ok ->\n\
                     SupTuple = {{beamtalk_supervisor, '{sup_class}', '{sup_mod}', Pid}},\n\
         ...
         stop(_State) -> ok.\n",
        pkg_name = pkg.name,
        cb_module = cb_module_name,
        sup_mod = sup_module,
        sup_class = supervisor_class,
    );
