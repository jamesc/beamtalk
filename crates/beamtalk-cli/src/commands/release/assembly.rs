...
pub fn generate_vm_args(
    ...
    // `@localhost`, not the OS hostname: distribution is loopback-only by
    // design here (ADR 0125 §1.6, the `inet_dist_use_interface` line
    // below), and the launcher's client verbs (`stop`/`ping`/`rpc`) target
    // this identical literal — `localhost` always resolves (POSIX
    // guarantees an `/etc/hosts` entry for it), unlike a host's own
    // hostname, which some environments (observed on GitHub Actions
    // runners) never register anywhere resolvable. An unresolvable target
    // makes a client's `net_adm:ping/1` return `pang` indistinguishably
    // from a real auth/connection failure (ADR 0125 §1.7), which is what
    // made this so slow to track down. `-sname` also rejects a literal IP
    // (`127.0.0.1`) outright as "not a short name" (it contains dots), so
    // `localhost` is both the resolvable *and* the syntactically valid
    // choice here.
    let mut content = format!(
        "## Generated by `beamtalk release` — ADR 0125 §1.2/§1.6.\n\
         -mode interactive\n\
         -sname {release_name}@localhost\n\
