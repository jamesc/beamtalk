...
pub struct WorkspaceMetadata {
    ...
    pub created_at: u64,
    /// Project-identity fingerprint: the package `name` from the
    ...
    /// `project_path` only fires when this matches the current
...
/// URL-safe base64 rules out `+`/`/` anywhere; the reroll rules out a
...
/// partial startup that was interrupted.
///
/// **`startup.log` is intentionally preserved**: the retry helpers
...
pub fn remove_stale_runtime_files(workspace_id: &str) -> Result<()> {
    ...
    // NOTE: `startup.log` is deliberately NOT removed here.
...
mod tests {
    ...
    /// Verify that `get_workspace_metadata` returns a clear error for an empty file.
    #[test]
    ...
    /// including the `starting` tombstone, but preserves
    ...
    fn test_remove_stale_runtime_files_removes_starting_tombstone() {
        ...
        // (e.g. `start_detached_node`) delete it explicitly.
        assert!(
    ...
    /// message to reference valid diagnostics.
    #[test]
