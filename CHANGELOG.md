...
- Fix `beamtalk repl` (and therefore `beamtalk-mcp --start`) hanging on Windows when stdin is not an interactive terminal. The interactive rustyline loop is now entered only when stdin is a TTY; with redirected/null stdin the command bootstraps the detached workspace node, prints its port, and exits. On Windows rustyline never returned EOF for a redirected stdin handle (it does on Unix), so `beamtalk-mcp --start` — which runs `beamtalk repl --port 0` and waits for it to exit — blocked until its MCP `initialize` timed out. The interactive TTY path is unchanged (BT-2568).
