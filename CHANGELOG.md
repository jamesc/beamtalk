...
- **LiveView IDE packaging** — the IDE now ships on its own release lane, separate from the toolchain bundle. `just dist-liveview` builds an OTP release; per-platform archives (`linux-x86_64`, `macos-arm64`, `macos-x86_64`) publish via `liveview-release.yml`; a Docker image (`ghcr.io/jamesc/beamtalk-ide`) bundles Elixir + OTP + built assets. Version is derived from the repo `VERSION` file (BT-2513, BT-2514, BT-2515, BT-2516).
