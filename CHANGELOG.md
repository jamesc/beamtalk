...
- **`nil` and type patterns in `match:` (ADR 0107)** — `match:` arms now support a bare `nil` pattern (matches nil exactly, narrows subsequent arms to exclude `Nil`) and a `binding :: ClassName` type pattern (tests the scrutinee's runtime class and binds it narrowed to that class in both the `when:` guard and the body). Supported classes: all primitives (`String`, `Integer`, `Float`, `List`, `Dictionary`, `Boolean`, `True`, `False`, `Symbol`, `Block`, `Pid`, `Reference`, `Port`), `Nil`/`UndefinedObject`, user-defined `Value` subclasses, and `Actor` subclasses. `Supervisor`/`DynamicSupervisor` subclasses are supported (BT-2870, #2987). A `Symbol` type pattern excludes `nil` and booleans. Subsequent arms see the scrutinee type narrowed by difference (e.g. after `s :: String`, remaining arms see the type minus `String`). Multiple pattern kinds (literal, nil, type, wildcard) can be mixed freely in one `match:` (BT-2854, BT-2855, #2954, #2959).
- Type-pattern parser diagnostics polish — generic type arguments in a type pattern (`items :: List(Printable)`) now produce a single, targeted "generic type arguments are not supported in a type pattern yet" diagnostic instead of a parse-error cascade; a stray type annotation on a `nil` pattern (`nil :: String`) and a non-identifier after `::` (`x :: 5`) likewise collapse to one clear diagnostic each (BT-2860).
- **Value subclass auto-accessors reject case-insensitive slot name collisions** — two slots on the same `Value subclass:` whose names differ only by the case of their first letter (e.g. `field: x` and `field: X`) now produce a compile error, since both would generate the same `withX:` setter selector (BT-2830, #2940).
- Fix `check_state_defaults` silently skipping Union-typed default values — a field default that infers as a Union type is now checked against the declared field type, warning when no union member is compatible (BT-2848, #2949).
- Fix a lone singleton `Symbol` (e.g. `#first`) warning against an `atom()`-specced FFI parameter while the union form (`#first | #last`) passed — `check_ffi_argument_types` now uses the same `is_type_compatible` subtyping predicate in both the scalar and union arms (BT-3024, #3178).
- Fix cascade continuation messages (`c foo: 1; bar: "wrong"`) silently skipping argument type-checking on all messages after the first in the cascade (BT-2845, #2946).
- Fix nested Union type-args inside a `Known` type (e.g. `Array(String | Nil)`) auto-passing return-type validation instead of recursing — a method declared `-> Array(String)` whose body infers `Array(String | Nil)` now correctly warns (BT-2847, #2945).
- **Return-type checking now covers `Union`/`Dynamic` method bodies in `typed` classes** — `check_return_type` previously bailed out silently whenever a method body inferred as `Union` or `Dynamic`, even against a concrete declared return type (the exact gap that let BT-2824's bug go unnoticed). A `Union` body with an incompatible member now raises the same type-mismatch warning a `Known` body would (e.g. `foo -> String => self bar ifTrue: ["yes"] ifFalse: [42]`, body `String | Integer`); a legitimate branch-union body that matches the declared union (e.g. BT-2047's `ifNil:ifNotNil:` pattern) stays silent. A `Dynamic` body whose reason is `AmbiguousControlFlow` or `UnannotatedReturn` — reasons that mean the checker *could* have inferred something better — now raises a hint against a declared concrete return type; genuinely dynamic reasons (`DynamicReceiver`, `UntypedFfi`, `DynamicSpec`, `Unknown`) stay silent. Both are scoped to `typed` classes; use `@expect type` to suppress a specific site. Fixed in passing: a declaration-level `@expect` (placed before a method/state declaration rather than as the first statement inside a body) written after another method's body was silently swallowed into that preceding method's body instead of attaching to the method it precedes — along with its own doc comment and leading `//` comments, which vanished entirely with no diagnostic (BT-2829). Follow-up: the `Union`-vs-declared-`Union` comparison now also checks generic type arguments (not just the base class), so e.g. a `Result(Integer, Error)` body is correctly flagged against a declared `Result(String, Error) | Nil` instead of silently passing on the `Result` base-class match alone (BT-2840).
...
- **`inspect` returns `Inspector`, not `String` (breaking)** — `anObject inspect` now returns a navigable `Inspector` cursor (`Inspector on: self`) instead of a debug string. The structural debug string is produced solely by `printString` (ADR 0094). Code that used `inspect` in string position (`++`, interpolation) must switch to `printString`; a transitional `inspect_in_string_position` lint flags these sites (BT-2504).
- **Singleton receivers resolve methods against `Symbol`** — a non-union singleton type (`#text`, `#infinity`) is a subtype of `Symbol` but was not itself in the class hierarchy, so message sends previously dropped DNU validation and return-type inference (falling through to `Dynamic`). The type checker now resolves singleton receivers through `Symbol`'s protocol, so `#infinity asString` infers `String` and an unknown selector produces a DNU hint naming the singleton the user wrote (e.g. `#infinity`) rather than `Symbol` (BT-2647, BT-2679).
- **Pure-atom enum precedence over `ok`/`error` Result recognition (type inference)** — the spec reader now infers a singleton union (`#text | #json | #xml`) for pure-atom FFI spec unions containing atoms beyond `ok`/`error`, instead of applying ADR-0076 Result recognition. Bare `ok`/`error`-only unions and tuple-form Results are unaffected. Runtime `ok`/`error` coercion is unchanged (BT-2647).
- **FFI `undefined` maps to `#undefined`, not `Nil`** — Erlang `undefined` in an FFI return type spec maps to the singleton `#undefined` (a `Symbol`), not `Nil`. The FFI boundary does not coerce `undefined` → `nil`, so mapping the type to `Nil` would be unsound (BT-2647).
...
- **Erlang `string()` / `nonempty_string()` FFI params accept `String`** — the auto-extracted FFI type mapping for classic charlist-typed Erlang params (e.g. `os:putenv/2`'s `VarName`/`Value`, `os:find_executable/1`'s `Name`) now widens to `String | List` instead of `List` only, mirroring the existing `binary()` → `String | Binary` treatment. Previously `beamtalk lint` and the compiler's own diagnostic pass flagged standard calls like `Erlang os putenv: "X" value: "y"` as type mismatches even though the underlying OTP call accepts binaries fine (BT-2817).
...
- Fix cascade-bodied method immediately followed by a keyword method corrupting the following method's parameter bindings — the cascade-message keyword loop now applies the same boundary check as `parse_keyword_message()`, preventing it from consuming the next sibling method's leading keyword token (BT-2811).
- **`Boolean` is now `sealed`** — `Boolean` was `abstract` but not `sealed`, leaving the closed-world assumption behind solo `ifTrue:`/`ifFalse:` inference (BT-2868) structurally unenforced. User code could not meaningfully subclass `Boolean` before, so there is no behavioral breakage (BT-2886, #3004).
- **Solo `ifTrue:`/`ifFalse:` on `Boolean` infers `Boolean | R` instead of `Dynamic`** — a solo `ifTrue:` or `ifFalse:` send on an unnarrowed `Boolean` receiver now infers the union of the `Boolean` self-branch and the block's return type (`Boolean | R`), instead of collapsing to `Dynamic(DynamicReceiver)`. Mirrors the BT-2824 `ifNil:`/`ifNotNil:` fix (BT-2868, #3001).
- **`Supervisor`/`DynamicSupervisor` subclasses supported in type patterns** — `x :: MySupervisorSubclass` in a `match:` arm now compiles and matches correctly at runtime, replacing the previous compile-time rejection. Codegen handles the `'beamtalk_supervisor'` 4-tuple runtime representation (BT-2870, #2987).
- **Block-literal callback params infer arg type from a Union's `Block(...)` arm** — when a method parameter is typed as a union containing a `Block(...)` arm (e.g. `Block(Request, Response) | Handler`), passing a block literal now infers the block's parameter types from the `Block(...)` arm instead of falling back to `Dynamic` (BT-2864, #2996).
- Fix constructor pattern `:: ClassName` tail producing a cascading "Expected '->' after pattern" parse error — now produces a single clear diagnostic (BT-2885, #2998).
- Fix `ifOk:ifError:`'s `R` type parameter being polluted by a non-local return (`^`) inside the `ifError:` block, causing downstream sends to fail with incorrect types (BT-2866, #3000).
- Fix `isKindOf:` check against a `Dynamic`-typed value falsely diagnosed as "can never be true" (BT-2865, #2999).
...
- **`internal type` modifier for type aliases (ADR 0108)** — `internal type Foo = ...` marks an alias as package-private, following the same model as class- and method-level `internal` (ADR 0071). An `internal` alias from a different package is never seeded into a consumer's alias table; a public signature referencing an internal alias — including transitively through another alias's expansion — is a compile error (E0402) (BT-2898, #3022).
- **Cycle detection for type alias references** — cyclic alias references (e.g. `type A = B | Integer` / `type B = A | Symbol`) are detected at declaration time with a clear diagnostic (`type alias cycle: 'A' → 'B' → 'A'`). Aliases are resolved in dependency-topological order (BT-2896, #3021).
- **Named Erlang `-type` emission for type aliases** — codegen emits named Erlang `-type` attributes from `type Name = ...` declarations, so Erlang/Elixir FFI consumers get idiomatic Dialyzer types. Method annotations referencing an alias name now emit `user_type` references in production `-spec` attributes instead of falling through to `any()`. Module-local aliases are resolved; cross-package alias references are a follow-up (BT-2932) (BT-2900, BT-2909, #3018, #3040).
- Fix `check_state_defaults` not resolving type alias names in `field: x :: AliasName = #literal` default-value checks — the declared type is now expanded through `resolve_type_annotation` before comparison (BT-2923, #3038).
- Fix `sort:` with nested mutating comparators stomping shared process-dict state — each invocation now generates a fresh `erlang:make_ref/0` key instead of a fixed atom (BT-2948, #3089).
...
- **Declaring an equality operator override is a compile error** — `=:=`, `=/=`, `==`, and `/=` lower straight to Erlang BIFs with no class lookup, so a class-level method for any of them is unreachable dead code. Declaring one is now rejected at compile time (`@primitive`/`@intrinsic` pragma bodies are exempt). Use `Object>>equals:` for overridable content-based comparison (BT-2997, #3187).

### Standard Library

- `List>>detect:` now raises `not_found` (instead of `does_not_understand`) when no element matches the predicate; `List>>from:to:` raises `index_out_of_bounds` for a start index below 1, matching `at:`. Both use purpose-specific error kinds so `on:do:` can distinguish a semantic failure from a genuinely misspelled selector (BT-3025, #3179).
- **`detect:` raises `not_found` on every collection** — `Set`, `Bag`, `Dictionary`, `Interval`, `String`, and `Stream` now raise `not_found` when no element matches, matching `List`. `detect:ifNone:` is the non-raising alternative; `Stream` gained `detect:ifNone:` (it had none). The error names the receiver's class (BT-3028, #3186).
- **`Object>>equals:` — overridable content-based equality** — new method on `Object`, defaulting to `self =:= other`. Linear scans honour it — `includes:` on Collection/List/Array/Dictionary, `List>>indexOf:`, `TestCase>>assert:equals:` — while keyed containers (`Set`, `Dictionary` keys, `List>>unique`) keep `=:=`. An override may only make more values equal, never fewer (BT-2997, #3187).
- Fix `Set` and `List>>unique` comparing elements by term order (`==`), collapsing `1` and `1.0` — both now use `=:=`, agreeing with `Dictionary` keys and `List>>includes:` (BT-2997, #3187).
- **`Json.parse:` declares `Result(JsonValue, Error)` instead of bare `Result`** — `Json.parse:` now returns a fully-parameterized `Result(JsonValue, Error)` with a new `type JsonValue = Nil | Boolean | Integer | Float | String | List | Dictionary` alias, improving type inference for JSON parsing call sites (BT-2827, #3037).
- **Stdlib named type aliases for singleton unions** — four anonymous `Symbol` singleton unions in stdlib are replaced with named `type` aliases: `SupervisionStrategy` (`Supervisor.strategy`), `EtsTableType` (`Ets.new:type:`), `LogLevel` and `LogFormat` (`BeamtalkInterface.logLevel`/`logFormat`). Hover and diagnostics now show the alias name instead of the raw member list (BT-2923, #3038).
- **`RestartStrategy` and `Timeout` stdlib type aliases** — `Actor.bt` declares `type RestartStrategy = #permanent | #transient | #temporary` and `TimeoutProxy.bt` declares `type Timeout = Integer | #infinity`. Several stdlib method signatures (`Actor>>supervisionPolicy`, `Actor>>withTimeout:`, `TimeoutProxy>>setTimeoutMs:`, `SupervisionSpec` restart field and `withRestart:` overrides) now use the named aliases instead of inline unions (BT-2939, #3083).
- **Stdlib type aliases auto-resolve in the REPL** — a fresh REPL session now knows about stdlib-declared type aliases out of the box. `::` typed locals referencing stdlib aliases resolve without declaring them first, and `:help RestartStrategy` shows `Declared in: stdlib`. A session-declared `type` with the same name shadows the stdlib alias (BT-2938, #3081).
- **Stdlib block callback parameter types tightened** — bare `Block` parameters on foundational methods (`whileTrue:`, `whileFalse:`, `ensure:`, `ifTrue:`, `ifFalse:`, `and:`, `or:`, `timesRepeat:`, `after:do:`, `every:do:`, `unwrapOrElse:`) are now parameterized (e.g. `Block(R)`, `Block(Boolean)`), improving type inference through block-passing call sites. No behavioral change — annotations are erased at codegen (BT-2834, #2944).
- **`Stream>>collect:` propagates transform return type** — signature changed from `Block(E, Object) -> Stream` to `Block(E, R) -> Stream(R)`, so the transform block's return type flows into the resulting stream's element type, matching `List>>collect:` (BT-2835, #2943).
- **`asJson` serialization hook for `Json generate:` / `prettyPrint:`, plus a `JsonRepresentable` protocol** — custom classes can now control their JSON representation by implementing an `asJson` instance method that returns a JSON-representable value (typically a `Dictionary` with explicit wire-format keys); any class implementing it structurally conforms to the new `JsonRepresentable` protocol (mirroring `Printable`). Both generators dispatch to the hook for any non-built-in value (`Value` instances, actors) and convert the result recursively, so nested `asJson` objects compose. A cycle — an object reappearing inside its own JSON output, directly or through intermediate objects — raises a `type_error` instead of exhausting the stack. Classes without `asJson` keep the existing behaviour — tagged maps encode their fields directly, and unsupported types raise a `type_error` whose hint now mentions the hook (BT-2818).
- **`Logger setLevel:` removed (breaking)** — deprecated since 0.2.0 with a one-time runtime warning. Use `Beamtalk logLevel:` instead; sending `setLevel:` now raises a standard `does_not_understand` error. The `beamtalk_logger` Erlang module (whose only remaining purpose was this shim) is deleted (BT-2790).
...
- **`ChangeLog` pending view collapses to one row per method** — `Workspace changes` (and its `size`/`do:`/`isEmpty`/`notEmpty`, the LiveView IDE Changes pane, and MCP `list_changes`) now shows only the latest active entry per `(class, selector)`. Repeated patches — or a patch followed by a `revert:`, which is itself a patch (ADR 0082 "Undo") — previously produced multiple pending rows for the same method, each with its own revert affordance. The default view now mirrors what `Workspace flush` applies and what `dirtyMethods` groups; superseded ("shadowed") entries remain reachable for audit via `ChangeLog allEntries` / `select:` and the new `ChangeEntry isShadowed` predicate (BT-2574).
- **`ChangeLog` net-vs-disk diff + "disappear when clean"** — each pending entry is now compared against its current on-disk body. A method reverted back to exactly its on-disk state has no net change and drops out of `Workspace changes` entirely (the audit entry stays in `allEntries` / `select:` via the new `ChangeEntry isClean` predicate). Entries that still differ carry `ChangeEntry diff` — a net on-disk→in-memory unified diff (lines prefixed `  ` / `- ` / `+ `), generated once in the runtime so REPL, MCP, and the LiveView IDE render identical text. The LiveView Changes pane shows the diff in an expandable disclosure per row. `Workspace changes` now reads as "what differs from disk", not "everything touched this session" (BT-2575).
...
- **System `Announcement` subclasses (ADR 0093 Phase 3)** — well-known typed system events the runtime publishes on `SystemAnnouncer current`: `ActorSpawned` / `ActorStopped` (actorClass, pid, reason), `ClassLoaded` / `ClassRemoved` (className), `BindingChanged` (name, value, sessionId), `FlushCompleted` (files), `ObjectStateChanged` (pid, actorClass, changedSlots), and `SupervisionChildAdded` / `SupervisionChildCrashed` (supervisor, childClass, childPid, reason). A tool subscribes once and filters by event class instead of wiring four bespoke notification channels — e.g. `SystemAnnouncer current when: ActorSpawned do: [:e | Transcript showLine: e actorClass asString]` (BT-2445).
...
- **`Inspector` / `InspectorField` — navigable object inspection (ADR 0095)** — new `Inspector` class provides an immutable cursor over any object: `Inspector on: anObject` (or the shorthand `anObject inspect`). Supports four `kind`s — `#value` (structural slots), `#actor` (timeout-guarded state snapshot), `#collection` (windowed — page size 50, `page:` navigation, cheap `size`), and `#foreign` (non-Beamtalk OTP processes via `process_info`). Navigation messages (`at:`, `fields`, `parent`/`root`/`path`, `refresh`, `printString`/`printStringExpanded:`) return new cursors. `evaluate:` runs expressions with `self` bound to the inspected value (values only; actors return `Result error:`). Cross-surface wire form via `asDictionaries`/`asDictionary`. `InspectorField` is an immutable value record with `name`, `label`, `value`, `kind`, and `drillable` (BT-2502, BT-2503, BT-2505).
- **Inspector performance** — Dictionary `size` is O(1) (was full sort), Dictionary/Bag `at:` is O(log n) via `maps:find` (was O(n)), Array windowing reads only the page slice (was whole array). `at:` uses exact `=:=` matching, aligning with `Dictionary >> at:` (BT-2507).
- **`FlushCompleted` announcement** — new system event emitted when `Workspace flush` finishes writing source files. `files` field lists the affected file paths (BT-2530).
- **`BindingChanged.sessionId` field** — `BindingChanged` now carries the originating session's protocol id (`nil` outside a session worker) (BT-2530).
- **`ObjectStateChanged` announcement** — new opt-in system event published when a *watched* actor commits a state write (ADR 0095 §5). Fields: `pid`, `actorClass`, `changedSlots`. Publishing is gated by `beamtalk_object_watch` subscriptions — unwatched actors pay only a single ETS membership read (BT-2489).

### Runtime

- Fix a raw non-UTF-8 binary (crypto output, hashes, compressed data, `Uuid asBinary`) crashing the entire BUnit test VM instead of failing one test — `.size`/`.class` on such a value dispatched through `String`'s grapheme-aware path, whose `badarg` reason embedded the raw bytes in the failure message, which then threw `{invalid_byte, N}` out of `json:encode/1` in the runner's boot process. Three fixes: a binary that isn't valid UTF-8 now answers `Binary` from `class` and dispatches to `Binary`'s byte-level methods (`(Erlang crypto strong_rand_bytes: 16) size` is `16`, not a crash); `printString` renders such bytes as `<<FF FE FD>>` hex instead of embedding them raw; and `beamtalk_test_runner:result_to_json/1` sanitizes error messages and degrades to a counts-only document rather than throwing. Valid UTF-8 binaries are unchanged — they stay indistinguishable from `String` and keep grapheme semantics (BT-2999).
- Fix non-ASCII characters in Erlang binary literals silently truncating to their low 8 bits — typographic em-dashes in error hints reached the user as invisible control characters. All 42 offending sites across 17 runtime modules replaced with ASCII. A new `just lint-binary-literal-encoding` lint (wired into `just lint` / `just ci`) rejects non-ASCII in binary literals lacking `/utf8` (BT-3026, #3177).
- Fix a `Collection` subclass whose `do:` delegates to a **class-side** method silently answering empty results — `asList` returned `#()`, `inject:into:` returned the seed, and `sum`/`min`/`max`/`average` followed, with no error raised. `beamtalk_collection:to_list/1` accumulated in the process dictionary, but class-method dispatch runs the callee in the class gen_server process, so the iteration block's writes landed in the wrong process; it now accumulates through the caller's mailbox and is process-boundary agnostic. Two related leaks are fixed with it: `Collection>>includes:` (and any user `^` through such a `do:`) no longer surfaces a raw `{'$bt_nlr', ...}` tuple — non-local returns are relayed back across the boundary and unwind the enclosing method normally — and the re-entrant case (a class method iterating a collection whose `do:` sends back to that same class) raises a structured `dispatch_error` naming the selector instead of a bare `{calling_self, {gen_server, call, ...}}` (BT-3022).
- **`beamtalk_version:get/0`** — new workspace-side RPC target reporting a structured version map (`runtime_version`, `protocol_version`, `otp_release`, `erts_version`) for the desktop-attach `/readiness` version handshake (ADR 0097). Callable via `:rpc.call(WorkspaceNode, beamtalk_version, get, [])`, the same way the front already calls into `beamtalk_repl_ops`/`beamtalk_repl_protocol`. Purely additive — no existing RPC call sites changed (BT-2991).
- Fix `browse-type-aliases` not showing aliases from types-only packages — `beamtalk_package:all/0` now recognizes an OTP application as a Beamtalk package when it has a non-empty `type_aliases` env key, even with no `classes`. The System Browser, VS Code sidebar, and `browse-type-aliases` REPL op now enumerate these packages correctly (BT-2915, #3047).
- **Type alias hot-reload re-check** — redefining a `type Name = ...` alias at the REPL now triggers re-checking of all classes whose annotations reference that alias name (transitively). A `matchExhaustive:` proof that was valid under the old member set is invalidated when the alias gains or loses members. Backed by a new `beamtalk_alias_xref` index mapping alias names to dependent classes (BT-2899, #3029).
- Fix `Dictionary>>doWithKey:`/`keysAndValuesDo:` via `perform:` with a stateful block crashing with a raw arity error instead of a clear `stateful_block_dispatch` diagnostic — extends the guard to cover `doWithKey:` directly; `keysAndValuesDo:` inherits the fix transitively via self-hosting (`self doWithKey: block`) (BT-2913, #3087).
- Fix `do:`/`collect:`/`select:`/`reject:`/`inject:into:` on List/Collection crashing with a raw Erlang error when a stateful (Tier 2) block is passed via `perform:` — now raises a clear `#beamtalk_error{kind=stateful_block_dispatch}`. Pure (Tier 1) block behavior is unchanged (BT-2888, #3019).
...
- Fix `perform:`/`perform:withArguments:` on Block `whileTrue:`/`whileFalse:`/`repeat`/`on:do:`/`ensure:` raising the same wrong-selector `does_not_understand` as BT-2812 — pure (Tier 1) condition/receiver/handler/cleanup blocks now run a real generic loop or try/catch; stateful (Tier 2) blocks raise a clear `stateful_block_dispatch` error (BT-2908).
...
- **Legacy REPL wire format removed (breaking)** — the pre-0.5.0 `type`-field request format (`{"type": "eval", "expression": …}`) and `type`-keyed response envelopes are gone from `beamtalk_repl_protocol`; requests must use the op protocol (`{"op": …, "id": …, "session": …}`). Raw non-JSON input is still accepted as an eval expression, but responses now always use the op-protocol shape. The deprecated `parse_request/1` legacy entry point is also removed. All first-party clients (CLI, LSP, VS Code, LiveView) already speak the op protocol (BT-2789).
...
- Fix Inspector crashing on remote-node pids — `Inspector on:` now guards the node check and degrades remote pids to an unavailable `#foreign` cursor (BT-2508).
- Fix Inspector edge cases: REPL `bind: #self` no longer shadows the reserved `self`; empty list `#()` consistently treated as `#value` leaf; dead actor `refresh` stays `#actor` kind; forged collection maps degrade gracefully (BT-2509).
- **`beamtalk_object_watch` — per-object change subscription server** — new supervised gen_server enabling opt-in reactive Inspector tracking. `subscribe/2`/`unsubscribe/2` register a subscriber for a specific actor pid; on a committed state write the actor publishes `{object_changed, Pid, ChangedSlots}` to subscribers and an `ObjectStateChanged` system announcement. Monitors clean up on subscriber/actor death. Unwatched actors pay only a single message-free `ets:member/2` check (BT-2489).
...
- **Reload-triggered re-check** — hot-reloading a method now automatically re-checks dependent callers via xref and surfaces diagnostics when a method's signature changes or is removed, capped at 20 callers with a note when more exist (BT-2778).
- **Reload-induced diagnostics on all surfaces** — reload-triggered diagnostics (broken callers, stale signatures) now publish across all surfaces — CLI REPL inline notices with severity icons, LSP editor squiggles tagged `source: "beamtalk (reload)"`, and a new `reload_check` WebSocket push channel — with clearing-by-replacement semantics so findings auto-clear when the triggering change is reverted or superseded (BT-2779).
- **Shape-change re-check** — when a live class redefinition adds, removes, or retypes a `state:`/`field:` slot, the system re-checks dependent `spawnWith:` call sites and field-accessor sends and publishes advisory diagnostics via the reload-check infrastructure (BT-2780).
- **Reload-induced findings initial-read on connect** — LSP and CLI REPL surfaces now fetch existing reload-induced findings on connect/attach, so stale-caller diagnostics recorded before a client joined are visible immediately rather than only appearing after the next reload (BT-2801).
- Fix self-send forwarding into a method that raises (`self error: "..."`) crashing with `function_clause` / `internal_error` instead of propagating the real error — the self-dispatch error branch now destructures the `{Type, Reason, Stacktrace}` triple and routes it through `beamtalk_exception_handler:reraise/3` (BT-2816).
- Fix reload-triggered re-check findings for cap-evicted callers persisting forever after the triggering change was reverted — cap-dropped candidates now have their findings marked stale with a `not re-verified this reload` note (BT-2802).
- Fix reload re-check findings for owners whose source is unavailable or whose diagnostic round-trip failed/timed-out persisting as permanently current — skipped/failed owners now have their findings marked stale instead of being silently stranded (BT-2828).
- Fix remaining Tier 2 stored-block codegen gaps: bare `self.field value:` inside `do:`/`collect:` loops no longer crashes with `badarity` (BT-2813), argument-position tuple leaks in stored-block `value:` calls are eliminated (BT-2814), and named local-var captured mutations thread state back correctly (BT-2815).
- Fix dynamic dispatch of a stateful (Tier 2) block via `value` crashing with a raw `badarity` instead of the clear `stateful_block_dispatch` error — the codegen now arity-discriminates before applying, so stateful blocks that capture mutable state raise the established error message. Covers any self-hosted method applying a block parameter, including `eachWithIndex:` via `inject:into:` (BT-2914, #3099).

### Tooling

- **`beamtalk-desktop-broker` crate — desktop attach connection-broker core (ADR 0097 Phase 2)** — a plain Rust library implementing the process-supervision logic the desktop shell's connection broker needs: discovery of live workspaces under `~/.beamtalk/workspaces/` (real JSON parsing, epmd liveness), free-port allocation with conflict retry, `BT_ATTACH_NODE_SUFFIX` sname seeding matching the shipped Elixir front's format, an OIDC guard that refuses to spawn when `~/.beamtalk/ide.toml`/`BT_OIDC_*` is configured, spawning a front with the loopback/no-OIDC/entropy-seeded posture, a two-stage readiness probe (HTTP up, then `GET /readiness` — BT-2983) shared with post-attach monitoring, and PID-file orphan reaping hardened against PID reuse via process-start-time comparison. Workspace create/stop shells out to the installed `beamtalk` CLI rather than bundling the toolchain. No GUI dependency — a shell (picker UI: BT-2986; packaging: BT-2987/BT-2988) builds the window/event-loop layer on top. The shared epmd-query logic previously duplicated in the `beamtalk workspace` CLI command moved to `beamtalk-workspace::epmd` so this crate and the CLI use one implementation (BT-2985).
- **`beamtalk-desktop-shell` crate + `desktop/` Tauri picker app — desktop attach connection-broker UI (ADR 0097 Phase 3, BT-2986)** — the picker/window-manager layer over `beamtalk-desktop-broker`. `beamtalk-desktop-shell` (a plain Rust library, workspace member, no GUI dependency) holds the shell-agnostic decisions: an atomic `decide_and_claim` closes the TOCTOU race the BT-2984 spike found and fixed in its own throwaway coordinator (two near-simultaneous attach clicks for the same workspace both observing "nothing tracked" and both spawning) so attach-twice reliably focuses/reuses the existing window rather than racing a second front, window-per-workspace label bookkeeping, and first-run empty-state classification (workspace list vs "create a workspace" via the installed CLI vs setup instructions when the CLI can't be resolved). `desktop/` wires that logic into a native Tauri app: a picker window listing live workspaces with liveness badges, attach with loading/probing state from the two-stage readiness probe, detach/quit that terminates the front process, post-attach `/readiness` re-polling reflected as a window-title prefix and a picker-side badge on disconnect (not a wall of LiveView errors), and the single-instance policy via `tauri-plugin-single-instance`. `desktop/src-tauri` is **not** a Cargo workspace member (needs `webkit2gtk-4.1`/`glib`/`gtk3` dev packages and the `cargo-tauri` CLI, unavailable in every environment that builds the rest of the repo) — see `desktop/README.md` for what was and wasn't possible to verify without a display server or that toolchain, and for the honest status of the still-open Tauri-vs-coordinator shell decision (BT-2984 spike criterion (e) remains desk-research-only). Wiring a CI build lane for it is BT-2987's job.
- **Desktop app packaging: Linux + macOS build lane (ADR 0097 Phase 4, BT-2987)** — `.github/workflows/desktop-release.yml` (mirroring `liveview-release.yml`) builds the desktop picker for Linux x86_64 (`.AppImage` + `.deb`) and macOS arm64 + x86_64 (`.dmg` + `.app`), bundling a freshly-built `bt_attach` release (`just dist-liveview`, `bin/server` unmodified) into the app as a Tauri resource. On macOS, every nested Mach-O binary in that bundled release (`beam.smp`, `epmd`, NIFs) is code-signed with Hardened Runtime entitlements (`scripts/ci/sign-macos-nested-binaries.sh`, `desktop/src-tauri/entitlements.plist`) before the top-level `.app` is built and signed/notarized — Tauri's own bundler only reliably signs the top-level bundle, not arbitrary nested resource binaries. Signing/notarization is conditional on Apple secrets being configured; their absence yields an unsigned artifact instead of a failed build. CI records `cargo tauri build` time and artifact size per platform to `$GITHUB_STEP_SUMMARY` as a regression baseline. Not yet wired into `release.yml`'s `on: release` path — the app hasn't compiled end-to-end in any available sandbox (see `desktop/README.md`), so this lane runs via `workflow_dispatch`/`workflow_call` until a real build confirms it.
- **Package registry (ADR 0070) — `name = "X.Y.Z"` registry dependencies, `beamtalk deps` registry support, and the `beamtalk version` / `beamtalk publish` author workflow** — a `beamtalk.toml` dependency can now name a bare version string (`yaml = "0.2.1"`) instead of `path`/`git`, resolved through a *registry index*: a git repository (or local directory, for testing) holding one `packages/<name>.toml` file per package, each listing that package's published `(version, git url, tag)` releases. The index location resolves through `BEAMTALK_REGISTRY` (env) → `[registry] url` (manifest) → the default index (`https://github.com/jamesc/beamtalk-registry`); once resolved, a registry dependency flows through the existing git-dependency machinery unchanged, so it locks to an exact commit SHA in `beamtalk.lock` (which now also records the requested `version` for registry entries) just like a git dependency does. `beamtalk deps add NAME` with none of `--path`/`--git` resolves NAME through the registry — pinned to `--version` if given, else the latest published release; `beamtalk deps update` re-resolves registry deps to a fresh SHA the same way it does for git deps (the pinned version itself only changes via a manifest edit). On the author side, `beamtalk version [X.Y.Z | bump patch|minor|major]` shows/sets/bumps the `[package] version` field in `beamtalk.toml`, and `beamtalk publish` tags the current version, pushes the tag to `origin`, and appends the release to the registry index (creating `packages/<name>.toml` on a package's first release) — the full author loop is `beamtalk version bump minor && git add beamtalk.toml && git commit -m "release 0.3.0" && beamtalk publish`. See `docs/beamtalk-packages.md` for the full syntax, index format, and a copy-pasteable template for hosting your own registry (BT-2978, BT-2979, BT-2980, #3123, #3125, #3126).
- **Registry index cache moved to a shared, user-level location, with advisory locking against concurrent access** — a git-backed registry index is now cloned once per *registry* into `~/.beamtalk/registry/<hash of the URL>/` instead of once per *project* into `_build/registry/index/`, so every project pointing at the same registry shares one clone (matching Cargo/Gleam) instead of each `beamtalk clean --all` or cold CI cache forcing a full re-clone. `BEAMTALK_REGISTRY_CACHE_DIR` overrides the cache root — a hash of the registry URL is still appended underneath it, so two different registries pointed at the same override never clobber each other's clone — e.g. set it to `_build/registry` for a per-project cache close to the previous layout. Every clone/refresh/swap now takes an exclusive advisory file lock scoped to the registry's cache directory, so a CLI `build` racing the LSP server or an MCP `lint` against the same registry can no longer interleave and observe a partially-swapped index (previously reported as a false "package not found"). `beamtalk clean --deps` now also clears a project-local registry index override, previously reachable only via `--all`. A registry error listing available packages after a not-found lookup now caps how many package names it reads from disk instead of materialising the whole index on every typo (BT-2996).
- **`beamtalk.lock` is now enforced on a fresh git dependency clone, not just a reused checkout (breaking)** — `beamtalk deps` previously compared a locked SHA against the dependency's checkout only when that checkout already existed on disk; a fresh clone (every CI runner, every `beamtalk clean`) skipped the comparison entirely, so a moved git tag silently resolved to whatever commit it currently pointed at and rewrote `beamtalk.lock` to match, with no error or warning. A fresh clone whose resolved SHA no longer matches a lock entry for the same `(url, reference)` now fails with an error naming the dependency, both SHAs, and `beamtalk deps update <name>`. This applies uniformly to tag, branch, and rev dependencies — a locked branch dependency is now reproducible up to the next explicit `beamtalk deps update`, rather than silently following the branch tip on every fresh build. `beamtalk deps add` and `beamtalk deps update` are unaffected: both intentionally re-resolve without consulting the existing lock (BT-2992).
- Fix `beamtalk publish` telling a user to bump the version after a partial failure — if a previous `publish` pushed the release tag but died before updating the registry index, the tag exists on `origin` but the version isn't actually published yet. A retry now checks the index before checking the tag, recognizes this as a partial failure (not a republish), and names the manual recovery steps for the index instead of suggesting a version bump, which would have abandoned the already-tagged release. A tag-existence check against an unreachable `origin` now falls back to the local-only verdict when a local tag already gives an actionable answer, rather than surfacing a network error (BT-3000).
- Fix `beamtalk fmt` dropping blank lines between protocol method signatures — `beamtalk fmt` now preserves blank lines between instance-to-instance, class-to-class, and instance-to-class boundary signatures in `Protocol define:` bodies. A blank line before the very first signature is still dropped (BT-2946, #3075).
- Fix `beamtalk fmt` dropping a blank line at module section boundaries — a blank line placed between the declarations section, standalone methods (`Class >> sel => body`), and top-level expressions is now preserved (BT-2943, #3074).
- Fix `beamtalk test` not enforcing E0401/E0402 `internal` visibility checks — BUnit test/fixture/doc-test compilation now resolves `current_package` per file from the pipeline's discovered-package map, closing the parity gap with `beamtalk build`/`lint` (BT-2922, #3099).
- Fix LSP startup-ordering race where a `didOpen`/`didChange` notification racing `initialized()` permanently stamped alias entries with the `$project` fallback package — `set_root_packages`/`mark_stdlib_file` are now safely re-appliable and re-stamp already-tracked entries (BT-2961, #3099).
- Fix stale `gen-native` subcommand references in user-facing docs — `beamtalk-native-erlang.md`, `beamtalk-tooling.md`, and README now use the current `generate native` name (BT-2947, #3099).
- Fix `beamtalk fmt` not preserving trailing comments on `type` alias and `Protocol define:` declarations — `type Port = Integer // comment` and protocol declarations with trailing comments now round-trip losslessly through the formatter (BT-2906, #3046).
- Fix `beamtalk fmt` emitting type aliases before protocols before classes regardless of source order — interleaved `type`, `Protocol define:`, and class declarations now preserve their original source order after formatting (BT-2907, #3045).
- **LSP completions, go-to-definition, find-references, and hover for type aliases** — the LSP now offers alias names alongside class/protocol names in type-annotation completion, resolves go-to-definition to the `type Name = ...` declaration span, enumerates all reference sites (including inside other aliases' RHS), and shows `AliasName (expansion)` on hover. Cross-package `internal` visibility filtering applies. A `window/showMessage` warning fires when workspace preload hasn't finished, so incomplete results aren't mistaken for exhaustive (BT-2901, #3030).
- **REPL accepts `type` declarations and `:help <Alias>`** — `type Name = ...` declarations are accepted as REPL input and persist for the rest of the session; declaring `type Direction = ...` echoes the bare name (`=> Direction`). `:help <Alias>` renders the declaration, its expansion, and any doc comment. Cross-turn alias resolution works for `::` annotations and `matchExhaustive:` (BT-2902, #3024).
- **REPL tab-completion for type alias names in annotation position** — completing after `::` at the REPL now offers type alias names from the current session alongside class/protocol names, matching the LSP's existing behavior (BT-2918, #3035).
- **System Browser and VS Code "Type Aliases" section** — the LiveView IDE System Browser gains a "Type Aliases" panel mode, and the VS Code Workspace Explorer sidebar gains a "Type Aliases (N)" section. Rows show alias name, expansion, doc comment, and package origin. Internal aliases from other packages are excluded (BT-2903, #3028).
- **Display-name provenance for hover and diagnostics** — when a type originates from a named alias, hover and diagnostics render `AliasName (expansion)` instead of the bare structural union. `check_impossible_singleton_comparison` now includes "did you mean" suggestions for near-miss singletons. A new `check_unresolved_type_aliases` validator flags annotations that closely resemble a registered alias name (BT-2897, #3023).
- **`beamtalk build`/`lint` now enforce `internal` visibility checks** — `beamtalk build` and `beamtalk lint` now correctly fire E0401/E0402/E0403 diagnostics for `internal` class, alias, and method visibility violations, which previously only fired in the LSP (BT-2920, #3033).
- Fix MCP `lint`/`diagnostic_summary` not enforcing E0401/E0402/E0403 `internal` visibility checks — the MCP analysis pipeline now sets `current_package` and stamps cross-file `ClassInfo`s with package metadata (BT-2921, #3036).
- Fix LSP find-references not firing the incompleteness warning and silently falling through to the identifier-only walker when the cursor is in type-annotation position but the declaring file hasn't been indexed yet (BT-2919, #3034).
- Fix `beamtalk fmt` silently deleting a class's leading doc comment when a `type` alias declaration with its own doc comment sits between the comment and the class (BT-2924, #3039).
...
- Fix FFI argument type-checking silently skipping Union-typed arguments — a Union argument to an FFI call is now checked member-by-member against the declared parameter type (BT-2846, #2950).
- Fix MCP `lint`/`diagnostic_summary` tools reporting false-positive `Unresolved class` diagnostics for classes defined in project dependencies — dependency class metadata is now resolved from on-disk `_build/deps/` checkouts (BT-2823, #2934).
- Fix spec-extraction worker missing the project's own native ebin dir on its code path, causing cross-referencing native Erlang modules to degrade to `Dynamic` FFI types (BT-2861, #2955).
- **Type cache invalidation on spec-reader changes** — the FFI type cache now carries a content hash of the spec-reader module; a mapping-logic change across compiler versions invalidates stale cache entries instead of silently returning outdated type mappings (BT-2852, #2957).
- **Pre-save advisory (`Behaviour precheckCompile:source:`) and `:recheck image` command** — `precheckCompile:source:` compiles a pending method edit and reports would-be-stale dependents without installing it (the editor "check before save" hook). `:recheck image` / `Workspace recheckImage` performs a full whole-image re-check of all live classes with recorded source. Both are exposed on REPL, MCP (`precheck_method` / `recheck_image`), and LSP (`beamtalk.precheckMethod` / `beamtalk.recheckImage`) (BT-2782).
- **`__doc__` / `__signature__` for value-type synthetic accessors** — compiler-generated slot getters, `with<Field>:` copy-setters, and keyword constructors on `Value subclass:` classes now carry runtime documentation and signature metadata, so the System Browser read-only pane, `Beamtalk help:`, and MCP `docs` show resolved signatures instead of bare selectors (BT-2734).
- **`[diagnostics]` severity table in `beamtalk.toml`** — packages can now override per-category diagnostic severity via an optional `[diagnostics]` section in `beamtalk.toml` (keys: kebab-case categories like `dnu`, `type`; values: `"off"` / `"lint"` / `"hint"` / `"warn"` / `"error"`). Precedence: `@expect` > table > built-in default > `--warnings-as-errors`. Hard structural errors have a severity floor and cannot be silenced (BT-2793).
- **Project-scoped compilation** — CLI package builds, lint with a package root, and LSP after workspace preload now resolve cross-file extensions and class hierarchies within a package, eliminating false DNU hints for multi-file projects. Packages with `[dependencies]` suppress completeness-dependent warnings until cross-package metadata support ships (BT-2794, BT-2795, BT-2796).
- **LiveView IDE: read-only pane for compiler-derived methods** — selecting a compiler-derived method (value accessors, `with<Field>:` setters, actor `new`/`new:`/`spawn`/`spawn:`) in the System Browser now opens a read-only tab showing the resolved documentation and signature instead of a blank editable buffer. The `browse-method-source` backend keeps `source = null` for synthetic methods but resolves `doc`/`signature` via the same hierarchy walk `help:` uses, so the browser and `:help` always agree (BT-2714, #2818).
- **LiveView IDE: System Browser handles Protocols** — a protocol (e.g. `Printable`, ADR 0068) is reified as a sealed abstract class object dispatched by the shared `beamtalk_protocol_object` module, which carries no package and no on-disk source. As a result protocols fell into the browser's "(uncategorized)" group, were mis-badged as `project` even for stdlib protocols, and showed no methods when selected. The `browse-classes` op now flags each protocol (`is_protocol`) so the browser groups them under a dedicated "Protocols" category (mirroring the "Tests" bucket); protocol rows resolve their origin/`source_origin` through the protocol's *defining* module — recorded in the registry at registration via a new codegen `module` key — so a stdlib protocol badges `stdlib` and a project one badges `project`; and `browse-protocols` surfaces a protocol's required members (registry reflection, no user code) under a `requirements` protocol bucket — its required instance methods on the instance side, its required class methods on the class side — instead of an empty list (BT-2615).
- **LiveView IDE: non-blocking git panel loads** — the Git panel's status and log reads now run off-socket via `start_async`, so opening the tab or refreshing never blocks other socket events (tab switches, saves, clicks) while git runs. With autoflush off, per-method saves skip the git refresh entirely since the on-disk working tree is unchanged (BT-2590).
- Fix `Workspace flush` corrupting source files when a method has a leading `///` doc comment — flushing an edit duplicated the doc comment, dedented it to column 0, and mangled the following method. The method-definition byte span (`resolve_method_span`) now includes the method's leading doc-comment block and its indentation, matching the compiler's stored method source, and `flush` re-indents the stored (canonical, column-0) body to the file's indentation before splicing. The span stays a verbatim, full-line slice — splicing it back is an exact no-op — so flush no longer reformats untouched code (BT-2577).
- **LiveView IDE: CodeMirror Workspace editor** — the Workspace/terminal eval input is now a real CodeMirror 6 editor (replacing the transparent-textarea-over-`<pre>` overlay), fixing caret/highlight drift and giving proper selection, history and scrolling. Beamtalk highlighting is a lightweight regex tokenizer that shares its rules with the existing overlay highlighter and paints CodeMirror decorations with the themed `.tok-*` classes, so the TweaksPanel palettes keep working. CodeMirror is bundled into `app.js` by esbuild, so the shipped release stays npm-free; the build now needs node (provisioned via `.tool-versions`/mise, CI `setup-node`, and the Dockerfile build stage). The method-editor tabs migrate in a follow-up (BT-2538).
- **Cockpit: browsing is editing; REPL pinned to the bottom** — selecting a method in the System Browser now opens it as an editable tab in the centre method editor (seeded with its image-accurate source), instead of appearing in a separate read-only "Method Source" pane below the Workspace dock. The dedicated read-only pane is retired (browsing *is* editing — the Smalltalk idiom), so the Workspace dock (REPL) is always the last element in the centre column and no longer gets shoved upward when a method is selected. The method editor now grows to fill the reclaimed vertical space rather than a fixed-height box, and the browser highlights whichever method the focused editor tab is showing.
...
- Fix `beamtalk repl` (and therefore `beamtalk-mcp --start`) hanging on Windows when stdin is not an interactive terminal. The interactive rustyline loop is now entered only when stdin is a TTY; for any non-TTY stdin (pipe, redirect, or null) the command bootstraps the detached workspace node, prints its port, and exits. On Windows rustyline never returned EOF for a redirected stdin handle (it does on Unix), so `beamtalk-mcp --start` — which runs `beamtalk repl --port 0` and waits for it to exit — blocked until its MCP `initialize` timed out. The interactive TTY path is unchanged; the only behaviour change is that a piped `echo expr | beamtalk repl` is now skipped rather than read to EOF (an undocumented path — non-interactive use is served by the port-based protocol and `beamtalk run`/`exec`) (BT-2568).
...
- **Cockpit UI (Phase 0/1) + System Browser backend** — the LiveView IDE is rebuilt to the cockpit UX design: tabbed IDE shell with Workspace/Transcript/Changes dock, docked Inspector with breadcrumb drill navigation, method editor with compile/save, and a tweaks panel (theme/accent/syntax-highlight/density/fonts via CSS variables). System Browser backend adds four browse ops (`browse-classes`, `browse-protocols`, `browse-method-source`, `browse-class-definition`) through `beamtalk_repl_ops` — pure reflection, no user code, Observer-grantable `:read` capability (ADR 0096, BT-2482).
- **LiveView IDE packaging** — the IDE now ships on its own release lane, separate from the toolchain bundle. `just dist-liveview` builds an OTP release into `dist-liveview/` at the repo root (and no longer leaves `phx.digest` artifacts in `priv/static/`); per-platform archives (`linux-x86_64`, `macos-arm64`, `macos-x86_64`) publish via `liveview-release.yml`; a Docker image (`ghcr.io/jamesc/beamtalk-ide`) bundles Elixir + OTP + built assets. The release includes a `bin/server` launcher: `bin/server <workspace-id>` resolves the workspace node + cookie from `~/.beamtalk/workspaces/` (like `just web`) and generates an ephemeral `SECRET_KEY_BASE` if unset. Version is derived from the repo `VERSION` file (BT-2513, BT-2514, BT-2515, BT-2516).
- **`inspect_in_string_position` lint** — new lint flags `inspect` used directly in `++` or string-interpolation position, since `inspect` now returns an `Inspector` cursor, not a `String` (BT-2504).
- **Cockpit Phase 2: System Browser pane** — the LiveView IDE gains a full System Browser with hierarchy view (class tree), category view (grouped by package), protocol/selector panes (instance/class side), method source viewer, and class definition viewer. Backed by the `browse-*` ops (ADR 0096) (BT-2491).
- **Cockpit Phase 3: Inspector live tracking** — the Inspector pane now tracks actor state changes reactively via `beamtalk_object_watch`: field-flash highlights changed fields, freeze/unfreeze toggle, pid stats chips (queue depth, memory, reductions, status), and poke (evaluate expressions against the inspected object) (BT-2492).
- **Cockpit Phase 3: floating inspector windows** — Inspector instances can be popped out into draggable overlay windows, enabling side-by-side comparison of multiple objects (BT-2493).
- **Cockpit Phase 3: Senders/Implementors popovers + omni search** — method editor shows Senders and Implementors navigation popovers (backed by the `nav-query` op via xref). Top-bar omni search over classes + selectors (backed by the `nav-symbols` op) with keyboard navigation (BT-2495).
- **`browse-protocols` real protocol categorization** — protocol bucket decision now layered most-authoritative-first: declared category → `synthetic` source fact → xref-extension provenance → Pharo-convention selector-name heuristic. CamelCase word-boundary matching prevents false positives (e.g. `address` ≠ `add`) (BT-2506).
- **`just doctor` health check** — new `just doctor` recipe runs `scripts/cloud-doctor.sh`, validating BEAM toolchain, Rust toolchain, rebar3, hex-bridge, and workspace connectivity (BT-2526).
...
- **Build artifact provenance (ADR 0098)** — `beamtalk build` now stamps each build scope (`_build/dev/` and each dependency) with the producing toolchain identity (`beamtalk_version` + compound OTP version). A toolchain mismatch forces a full rebuild instead of reusing stale artifacts. Compiled modules carry `beamtalk_version` and `otp_release` in `__beamtalk_meta/0` so a running workspace can detect staleness without re-reading the on-disk stamp. Workspace attach recompiles project sources on a project-stamp mismatch and fails with a directed `beamtalk clean --deps` message on a stale dependency (BT-2678).
- **Git repo toplevel caching** — `beamtalk_git` now resolves and caches the git repository toplevel once per project path instead of spawning `git rev-parse --show-toplevel` before every git operation, halving subprocess round-trips on the hot IDE read path (BT-2621).
- **LSP live-extracts Erlang FFI type specs** — the LSP now runs the same live `.beam` type-spec extractor as `beamtalk build` instead of only reading `_build/type_cache/` JSON files, so FFI argument-type diagnostics appear immediately on workspace open even before a build has run (BT-2859, #2995).
- Fix MCP `lint`/`diagnostic_summary` silently skipping Erlang FFI argument-type checks — the MCP analysis pipeline now loads the `NativeTypeRegistry`, matching what `beamtalk lint`/`beamtalk build` report (BT-2858, #2992).
- Fix MCP `lint`/`diagnostic_summary` producing false-positive `Unresolved class` diagnostics for classes defined in transitive dependencies — dependency resolution now walks the full `[dependencies]` graph (BT-2836, #2989).
- **`beamtalk type-coverage` and LSP hover/completion resolve FFI return types** — `NativeTypeRegistry` is now threaded through the full `infer_types` family (hover, signature-help, completion, type-coverage), so Erlang FFI call return types and their downstream expressions resolve correctly instead of showing `Dynamic(UntypedFfi)` (BT-2867, #3002).
...
- Add browser e2e test suite for the LiveView IDE (PhoenixTest + Playwright) covering editor, eval, inspector, keyboard shortcuts, and theme (#2529).
- CI: run the `:workspace` LiveView e2e suite alongside browser tests (BT-2510).
