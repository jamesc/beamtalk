...
- **Named Erlang `-type` emission for type aliases** — codegen infrastructure for emitting named Erlang `-type` attributes from `type Name = ...` declarations, so Erlang/Elixir FFI consumers get idiomatic Dialyzer types. Production call-site wiring deferred to BT-2909 (BT-2900, #3018).
