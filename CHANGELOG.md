...
- **Pure-atom enum precedence over `ok`/`error` Result recognition** — a pure-atom FFI spec union containing atoms beyond `ok`/`error` (e.g. `emergency | error | info | ...`) now narrows to a singleton union (`#emergency | #error | #info | ...`) instead of being intercepted by ADR-0076 Result recognition. Bare `ok`/`error`-only unions and tuple-form Results are unaffected (BT-2647).
