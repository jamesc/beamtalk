...
- **`asJson` serialization hook for `Json generate:` / `prettyPrint:`** — custom classes can now control their JSON representation by implementing an `asJson` instance method that returns a JSON-representable value (typically a `Dictionary` with explicit wire-format keys). Both generators dispatch to the hook for any non-built-in value (`Value` instances, actors) and convert the result recursively, so nested `asJson` objects compose. Classes without `asJson` keep the existing behaviour — tagged maps encode their fields directly, and unsupported types raise a `type_error` whose hint now mentions the hook (BT-2818).
