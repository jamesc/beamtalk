...
- Fix `Dictionary>>doWithKey:`/`keysAndValuesDo:` via `perform:` with a stateful block crashing with a raw arity error instead of a clear `stateful_block_dispatch` diagnostic — extends the guard to cover both 2-arg-block Dictionary iteration selectors (BT-2913, #3087).
