...
- Fix `findClass:` crashing with `erlang_exit` when a class process is transiently unavailable under heavy parallel load — `classesInPackage:` now returns `nil` for transiently-down processes (filtered out silently), matching the documented miss policy (BT-2467).
