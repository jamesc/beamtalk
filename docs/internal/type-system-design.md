...
Both operators are pure, normalising functions — most results collapse back into `Known`/`Union`/`Never`, so only the genuinely irreducible cases need new storage. `Negation` covers two co-finite flavours: singleton (Phase 1, `Symbol \ #foo`, base always exactly `Symbol`) and nominal-class (ADR 0102 §5, BT-2744, `Object \ Number`, base any nominal class, excluded a single strict subclass). The class/protocol composition ADR 0068 already specified (`Collection(Object) & Comparable`) is generalized into a second stored variant, `Intersection { members }` (Phase 2, driven by the `&` surface syntax below) — the irreducible case is class ∩ protocol, since class ∩ class always reduces via the hierarchy. `union_of` gained the matching absorption laws for the singleton flavour: adding an excluded member back to a `Negation` shrinks or clears `excluded` (`(Symbol \ #foo) | #foo ⇒ Symbol`), and two same-base negations union by *intersecting* their exclusions.
...
- **`class =` / `isKindOf:`** both get a principled *true* branch through `intersect` — a test against a hierarchy-unrelated class now narrows to `Never` and routes through the same impossible-comparison hint the singleton path already had (gated on inferred, not declared, provenance — a defensive `isKindOf:` guard against an unverified annotation stays silent). The *false* branch (ADR 0102 §5, BT-2744) is narrowed via `difference`, now hierarchy-aware — but **only for `isKindOf:`**: a proper-subclass test narrows the false branch to the nominal complement (`Number \ Integer`), an exact/supertype match narrows it to `Never`, and a hierarchy-unrelated class leaves it unchanged. `class =` tests *exact* class identity, whose negation ("not exactly `C`") doesn't rule out `C`'s subclasses the way `Negation`'s subtree-exclusion semantics require — narrowing it the same way would be unsound (a sealed subclass could satisfy `isKindOf: C` while failing `class =:= C`), so `class =`'s false branch stays unnarrowed, exactly as before this ADR. A `Negation`-typed receiver's method lookup and DNU checking resolve through its `base` — identically to a bare `base`-typed receiver — so the `isKindOf:` false branch stays fully conformance-checked, not a `Dynamic` hole.
...
- `x class =:= Foo ifTrue: [...] ifFalse: [...]` — narrows to `Foo` in the true block (via `intersect`); false branch stays unnarrowed — `class =:=` tests *exact* class identity, so a false result doesn't rule out a subclass of `Foo` (`Negation` can only exclude a whole subtree, which would be unsound here)
- `x isKindOf: Foo ifTrue: [...] ifFalse: [...]` — narrows to `Foo` (true) / `T \ Foo` (false), via `intersect`/`difference` — `isKindOf:`'s negation ("not `Foo` and not any subclass") does match `Negation`'s subtree-exclusion semantics
