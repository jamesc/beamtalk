...
- **Git panel** — post-flush VCS surface (ADR 0082 Amendment 1): working-tree status (branch/upstream/ahead/behind + per-file state), per-file unified diff, commit log, stage/unstage, commit, and revert file. Shell-out to system `git` (not a libgit2 NIF) so hooks/signing/credentials/config apply. Read ops are Observer-visible; mutating ops are Owner-gated via Facade RBAC. Cockpit surface-specific — CLI/terminal users use `git` directly
