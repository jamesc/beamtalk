...
| `runtime/apps/beamtalk_runtime/src/beamtalk_behaviour_intrinsics.erl` | New primitive functions. Unlike `classRemoveFromSystemByName/1` (which refuses before acting), these install unconditionally — no receiver-side check — following `classCompileSource/3`'s shape instead: side-dispatch to the instance/class gen_server call based on which metaclass tag `Self` carries, then let the ChangeLog-append layer derive `flushable`/`not_flushable_reason` from `sourceFile`, same as a patch. |
