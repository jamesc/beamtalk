...
2. **Local method table checked second** — selector defined in the class gen_server's own `instance_methods`/`class_methods` map → clear it (new gen_server call — see *Implementation*).
...
| **`sealed` methods** | The target method (local or extension) is declared `sealed` | **New rule, stricter than `removeFromSystem`'s precedent** (which checks module origin, not the `sealed` keyword, and so does not block removal of a user's own `sealed`-declared class or, symmetrically, any one method on it via `removeFromSystem`'s class-level teardown) — **open question below.** |

Removing a *contested* extension selector (two extenders defining the same selector on the same class) is allowed, not refused — it removes whichever extension currently wins dispatch, per *Extension methods* above; this is a behaviour clarification of an existing case, not a third refusal rule.

**Open question: is the `sealed`-method refusal correctly grounded, and does it need an escape hatch?** ADR 0032's actual text for why `Behaviour`'s own methods are sealed is narrower than "no self-deletion, ever": *"users cannot override the class protocol"* — a constraint on **third parties shadowing** the method, not on the method's own class deleting its own method. Deleting your own method is not overriding it. As written, this rule produces an asymmetry the ADR should not leave unexamined: `removeFromSystem` (BT-785) will delete an entire user-declared `sealed Object subclass: MyRegistry` class outright, but this ADR's rule refuses to delete one `sealed` method on that same class — you may destroy the whole thing but not part of it, with no `force:` escape hatch either way. The rule does have a genuine, stronger justification the text above doesn't make explicitly: ADR 0032 reserves "sealed method promotion" as a future dispatch-cache optimization (a cache keyed on "sealed ⇒ stable ⇒ never needs invalidation" would be unsound if a sealed method could later be removed) — that forward-looking argument, not the override guarantee, is the one worth keeping this rule for. This ADR takes no side: either (a) drop the `sealed`-method rule for non-stdlib classes and rely on the stdlib check alone, matching `removeFromSystem`'s precedent exactly, or (b) keep it, but re-ground the rationale on the sealed-method-promotion optimization path rather than the override guarantee, and decide whether an escape hatch is warranted. Left for reviewer input rather than decided unilaterally here.
...
**Not adopted:** the `undef_method` shape. "Subclass should not respond to this inherited selector at all" is a real, distinct operation this ADR does not provide, and — unlike the `tryRemoveSelector:` ephemeral variant deferred below — it is **not** symmetrically cheap to add later. An `undef`-style block requires a tombstone value in `instance_methods`/`class_methods` that `beamtalk_dispatch`'s chain walk must recognise and stop at (today every value in those maps is assumed to be `#{block := Fun, arity := N}`-shaped), and that tombstone must also be tolerated by the process-dictionary self-dispatch cache (`beamtalk_class_instance_methods`) and by `notify_hot_patch/1`'s traversal of `instance_methods`. This is a change to the method-table's value shape, not just a ChangeLog `kind` addition — the open `kind` enum reasoning that lets `tryRemoveSelector:` be deferred safely does not cover it. Scoping this ADR to `remove_method` semantics only and flagging `undef_method`-style blocking as a known, harder-than-it-looks future need (left as an open question below) keeps the tombstone question from being decided by omission.
...
- **Local (non-extension) selector removal** needs a new gen_server call as above, plus a call into `beamtalk_xref:purge_method(ClassName, ClassSide, Selector)` (already exists, already used by `beamtalk_extensions:unregister/2` for the extension case) so the per-method xref rows for *this method's own outgoing sends and references* don't outlive it. Incoming references — other methods' xref rows recording that they call this selector — are deliberately **not** touched; see the dangling-senders note in *Consequences* and *Implementation*.
- **Extension selector removal is not a complete reuse of `beamtalk_extensions:unregister/2` as it stands today** — two gaps this ADR must close, not inherit for free:
  ...
  - `index_extension_xref` registers an extension's xref rows under `{Class, false, Selector}` — the identical key an instance-side *local* method uses. For the local-shadowed-by-extension case in *Extension methods* above, `purge_method/3` on either one's rows will collide with the other's. Not a blocker (the shadowed local method is re-exposed but not re-indexed until its own next patch), but worth a code comment at the call site so the downstream implementer doesn't discover it via a debugger.
...
### Shared removal path with ADR 0082's `revert:`-of-an-add

ADR 0082's `Workspace changes revert: aMethod` already specifies that reverting an **add** (a method whose selector did not exist pre-patch) *removes* the just-added method, and that "a removal is the inverse of the add and does not append a new entry" — no ChangeLog module ships yet, so this path and `removeSelector:`'s new `{remove_method, Selector}`/`{remove_class_method, Selector}` gen_server handlers are being built around the same time, into the same maps. They must be the **one** removal path (CLAUDE.md's no-duplicate-implementations rule), not two independent ones — but they have different logging contracts: `removeSelector:` always attempts a best-effort ChangeLog append (see *ChangeLog interaction*), while `revert:`-of-an-add explicitly must **not** log a new entry. The resolution: the gen_server handler itself never logs — it only mutates the method tables and purges derived registries. ChangeLog appending is a caller-side concern that lives in `classRemoveSelector`'s primitive wrapper, not inside the shared handler, so `revert:` can call the same handler directly without triggering it.
