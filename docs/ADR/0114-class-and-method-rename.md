...
3. **`whereis_class/1` is a live per-call lookup, not a baked pid — except for already-instantiated objects.** `#beamtalk_object{}` carries its class as a direct pid (`element(4, Self)`, confirmed in `beamtalk_behaviour_intrinsics.erl`), established at instantiation time. This matters for correctness scoping: renaming a class does not put *existing instances* of that class at risk (their dispatch never re-resolves by name), only *other* code's compiled references to the old name.
