...
3. **`whereis_class/1` is a live per-call name lookup — but ordinary instance method dispatch never goes through it at all.** `#beamtalk_object{}` is `{class :: atom(), class_mod :: atom(), pid :: pid() | {registered, atom()}, ...}` (`beamtalk.hrl`); `local_call/3` (`beamtalk_object_class.erl`) pattern-matches straight to `class_mod` and calls `erlang:apply(Module, FunName, ...)` directly against the compiled module — no registry lookup by class *name* in that path at all. `class_mod` is bound once, at spawn time, and is a different thing from the `class` name `renameTo:` re-registers in `beamtalk_class_registry`. (`ClassPid = erlang:element(4, Self)` elsewhere in `beamtalk_behaviour_intrinsics.erl` is *not* evidence of a general instance-to-class pid reference — that pattern appears only in class-side intrinsics, where `Self` *is* the class object itself and its own `pid` field happens to hold the class gen_server's `self()`, per `beamtalk_class_dispatch.erl`'s `ClassSelf = #beamtalk_object{..., pid = self()}`. An ordinary instance's `pid` field is that instance's own actor pid, unrelated to its class.) This matters for correctness scoping: renaming a class does not put *existing instances* of that class at risk — their own methods dispatch via the `class_mod` binding fixed at spawn, not a name lookup — only *other* code's compiled references to the old *name* are at risk.
...
**The fix uses `recv_kind`, which the xref schema already carries, to split `sendersOf:`'s results into two tiers:**
...
{ts, seq, epoch, class, selector: "<new selector>", old_selector: "<old selector>",
 ...
 sites: [{sourceFile, span: {start, end}, source_ref, prev_source_ref}, ...],
   ...
   %% without receiver-type narrowing. These are the ONLY sites flush ever
   %% writes for this entry.
