...
- **The closure that's actually sound: `Counter`'s implementation must be the *only* implementation of the selector anywhere in `Counter`'s subclass tree.** Checkable directly against `beamtalk_xref_methods` (keyed by `{class, class_side, selector}`, ADR 0087) — query every class in `direct_subclasses:`'s transitive closure of `Counter` (excluding `Counter` itself) for a row matching the same selector/side. If none exists, no override can ever intercept a same-hierarchy `self`/`super` send before it reaches `Counter`, for any possible receiver, and auto-rewrite is sound. If *any* subclass overrides the selector, auto-rewrite is unsound for the whole selector — not just for sites downstream of that specific override — because a single shared call-site text is executed polymorphically across every possible receiver class, and safety must hold for all of them, not just the common case.
...
{ts, seq, epoch, class, selector: "<new selector>", old_selector: "<old selector>",
 ...
 sites: [{sourceFile, span: {start, end}, source_ref, prev_source_ref}, ...],
   ...
   %% that subclass tree also defines this selector (beamtalk_xref_methods
   %% check; an override anywhere means self/super dispatch can be
