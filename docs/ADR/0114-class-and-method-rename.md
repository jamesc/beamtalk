...
**The fix uses `recv_kind` *and* the site's `owner` field — both already in the xref schema (ADR 0087: every `Site` record in `beamtalk_xref_senders` carries `owner`, the class the sending method is itself defined in) — to split `sendersOf:`'s results into two tiers. `recv_kind: self_recv`/`super_recv` alone is not sufficient: it proves the send resolves *within the runtime receiver's own class hierarchy*, but not *which* hierarchy that is. A method defined on an entirely unrelated class can share the selector name and send `self`/`super` internally — e.g. `Timer>>increment` (no inheritance relationship to `Counter`) containing `self increment` — and `sendersOf: #increment` returns that site with `recv_kind: self_recv` even though it has nothing to do with `Counter`. Blindly auto-rewriting on `recv_kind` alone would silently corrupt `Timer`. Narrowing by `owner` closes this: the sending method's own defining class must itself be a member of `Counter`'s hierarchy for the send to even plausibly reach `Counter`'s implementation.**

- **Auto-rewritten (`confirmed` sites):** the definition itself, plus any site with `recv_kind: self_recv` or `recv_kind: super_recv` **whose `owner` is `Counter` itself or a transitive member of `Counter`'s subclass tree** — computed via the same `direct_subclasses:` closure `renameTo:`'s site discovery already builds (see above), reused rather than duplicated. Within that hierarchy, `self`/`super` dispatch is safe *by Smalltalk semantics, not by inference*: a `self #{oldSelector}`/`super #{oldSelector}` send inside a method owned by `Counter` or a subclass of it can only ever dispatch to `Counter`'s implementation or an override of it, the same way `super`'s target is already unambiguous to chain-walk dispatch (ADR 0032). An intervening override on some subclass between the sender's `owner` and `Counter` could in principle still resolve the send to that override rather than the exact method being renamed at that exact site — this residual ambiguity is accepted as the same *missed-reference*-adjacent risk ADR 0112 already tolerates (the rewrite still lands on a real override of the renamed method within the correct hierarchy, not on unrelated code), not a new false-positive category.
- **Reported, not rewritten (`candidate` sites):** every site with `recv_kind: other` (an arbitrary expression receiver — the common "external caller" case, and also the most common shape of a real, intended sender) or `recv_kind: erlang_ffi`, **plus any `self_recv`/`super_recv` site whose `owner` falls outside `Counter`'s hierarchy** (the `Timer` case above). The xref index cannot prove these target `Counter` rather than some unrelated class that happens to implement the same selector, so `renameSelector:to:` does not touch them. They are surfaced the same way ADR 0112's dangling-sender hint already surfaces risk — as a reported count and site list on the primitive's return value and the resulting ChangeLog entry — so the caller can inspect each one and, where it genuinely is a Counter-directed send, patch it manually via the ordinary `compile:source:` path.
...
{ts, seq, epoch, class, selector: "<new selector>", old_selector: "<old selector>",
 ...
 sites: [{sourceFile, span: {start, end}, source_ref, prev_source_ref}, ...],
   ...
   %% subclasses:, same closure renameTo: uses) — the only sender shape
   %% the xref index can attribute to this class's own hierarchy without
   %% receiver-type narrowing. A self_recv/super_recv site whose owner
   %% falls outside the hierarchy is NOT here — see candidate_sites.
   %% These are the ONLY sites flush ever writes for this entry.
 candidate_sites: [{sourceFile, span: {start, end}}, ...],
   %% other_recv/erlang_ffi sends found via the same sendersOf: query,
   %% plus any self_recv/super_recv send whose owner is outside this
   %% class's hierarchy — reported for human/agent review, never
...
| `runtime/apps/beamtalk_workspace/src/beamtalk_repl_eval.erl` / `beamtalk_repl_loader.erl` | One shared multi-site rewrite mechanism, generalizing ADR 0112's `remove_method/3` (a sibling `rewrite_sites/4` or equivalent). `renameSelector:to:` calls it with `sendersOf:`'s `self_recv`/`super_recv`-filtered results, further narrowed to sites whose `owner` is in the target class's hierarchy (`other`/`erlang_ffi`, and any `self_recv`/`super_recv` site outside the hierarchy, go to `candidate_sites`, never rewritten); `renameTo:` calls the *same* function with the union of `referencesTo:` and `direct_subclasses/1`. Each rewritten site's own class needs the same `purge_compiler_cache`/xref-reindex step `beamtalk_class_lifecycle` already applies to the renamed class (BT-3105/BT-3107) — not automatic, must be wired in explicitly. |
