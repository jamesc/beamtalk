...
- Any `.beam` with `abstract_code` on the code path gets auto-extracted types with meaningful keyword names from spec variable names
- Falls back to positional `with:` keywords when spec variable names are generic (e.g., single-letter `T`, `X`)
- Falls back to `Dynamic` when `.beam` lacks `+debug_info`
...
- **Imprecise auto-extracted types** — `term()` → `Dynamic` means some functions get no useful type info from auto-extract alone (mitigated by spec-derived parameter names making the output more useful even with `Dynamic` params)
...
- **Cold build cost** — first build must read `abstract_code` from `.beam` files across all modules on the code path. For large dependency graphs (200+ modules), this may add several seconds. Mitigation: (a) results are cached in `_build/type_cache/` and invalidated by `.beam` timestamp changes — incremental builds have zero extraction overhead; (b) `abstract_code` reading is fast (no source parsing — specs and param names come from the same chunk); **(c) BT-2469: parallel + memoized batch extraction.** `read_specs_batch/1` reads modules across `schedulers_online` worker processes, and a batch-scoped ETS table memoizes each remote module's parsed artifacts (`-type`/`-opaque` sets) so a referenced module like `sets` or `queue` is read and folded once per batch rather than once per referencing spec. Measured ~29× faster cold extraction of the 8 OTP apps (≈25 s → ≈0.9 s on 4 schedulers). **(d) BT-2470: shared OTP cache.** The OTP portion of the type cache (stdlib, kernel, erts, crypto, …) only changes when the OTP/ERTS version changes — it is not project-specific. OTP specs are therefore cached in a *shared, version-keyed* directory outside `_build/` — `<cache>/beamtalk/otp-specs/<otp_release>-<erts_version>/`, where `<cache>` is `$BEAMTALK_CACHE_DIR` or the platform cache dir (`$XDG_CACHE_HOME` on Linux). A freshly cloned workspace with an empty `_build/type_cache/` reuses this shared cache instead of re-reading hundreds of OTP `.beam` files (the dominant cold-start cost). Dependency and native module specs remain project-local. Shared-tier hits are mirrored back into `_build/type_cache/` so the LSP and `beamtalk lint` continue to load every module's specs from there unchanged, and cache writes are atomic (temp file + rename) so concurrent builds sharing the tier never read a half-written entry. An OTP upgrade lands under a new version key and never reuses stale-version entries. **(e) BT-2471: warm the shared cache proactively in remote sessions.** The shared tier from (d) only helps once it's warm — a Claude-Code-on-the-web container is cloned fresh and reclaimed after inactivity, so it starts every session cold. The `SessionStart` hook (`.claude/hooks/worktree-init.sh`) backgrounds `beamtalk warm-otp-cache` (a project-agnostic wrapper around `extract_type_specs`) so the shared tier is warm by the time the session's first interactive `beamtalk build` runs, without blocking session startup — see `docs/development/remote-sessions.md`
...
| 4 | BT-1852 | REPL: `:help Erlang <module>` + tab completion | M | Planned |
| 4 | BT-1853 | LSP: Hover, signature help, go-to-definition for FFI | M | Planned |
