...
- A warning is logged: `Extension String>>json overwritten (was owned by string_json, now owned by my_json_lib)`
- The `beamtalk_extension_conflicts` ETS table records all overwrites for debugging
- `beamtalk_extensions:conflicts/0` returns the full conflict history
...
Extensions are registered when their containing module's `on_load` callback fires. In compiled `.bt` files, this happens when the BEAM module is loaded. In the REPL, this happens immediately on evaluation.
...
### Negative
- Last-writer-wins can silently break code when load order changes — mitigated by conflict logging
...
### Neutral
- Extensions cannot add instance variables (state) to existing classes — this is by design, matching Swift/Kotlin/C# semantics
...
- Related ADRs: [ADR 0005](0005-beam-object-model-pragmatic-hybrid.md) (object model, extension registry design), [ADR 0006](0006-unified-method-dispatch.md) (dispatch chain), [ADR 0032](0032-early-class-protocol.md) (flattened table removal)
