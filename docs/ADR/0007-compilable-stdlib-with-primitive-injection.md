...
| **`docs/beamtalk-object-model.md`** (lines 1056-1089) | Section "Custom VM Primitives" says "You can't write primitives *in Beamtalk*" — this becomes false. Primitives are declared in Beamtalk via pragmas. The Smalltalk `<primitive: 142>` example should reference Beamtalk's `@primitive '+'` approach. | **High** — directly contradicts ADR 0007 |
| **`docs/beamtalk-object-model.md`** (lines 162-193) | Documents primitives as "sealed value types" with `Integer subclass: MyInteger // ERROR: Integer is sealed`. The `sealed` keyword doesn't exist yet — needs clarification that sealing is semantically true but syntactically enforced later (future work). | **Medium** — accurate in intent but misleading on syntax |
