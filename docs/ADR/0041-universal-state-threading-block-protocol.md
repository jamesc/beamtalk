...
2. **`StateAcc`** (Core Erlang map) — the compiler's state-threading map for mutation inside blocks. Generated by codegen, passed as `fun(StateAcc) -> {Result, NewStateAcc}`. This is what BT-842 universalizes.
