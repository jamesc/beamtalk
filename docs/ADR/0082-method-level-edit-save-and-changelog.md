...
| Undo | `Workspace changes revert: aMethod` undoes the most recent active ChangeEntry for that target. For a **modify** it re-installs `prev_source` (the prior body); for an **add** (a method whose selector did not exist pre-patch) it *removes* the just-added method, since the pre-patch state was "absent" (BT-2663). Both **instance-side and class-side** method patches are revertable symmetrically — the entry's `kind` (`instance`/`class`) carries the side, so a class-side modify re-installs the prior class-side body and a class-side add removes the class-side method (BT-2665). Reverting a **`new-class`** entry removes the just-created class from the live image (BT-2664). A modify whose prior body is genuinely unrecoverable (the recorded body is gone *and* the on-disk source no longer resolves) surfaces a loud error rather than silently deleting a method that existed before. The revert install is itself a patch (it appends a fresh ChangeEntry); a removal is the inverse of the add and does not append a new entry. Revert of a *modify* to its prior body needs a recoverable prior source (recorded `prev_source` or the current on-disk body); add-removal and new-class-removal need no prior body. |
