...
**`DynamicReceiver` narrowed (BT-2868):** `DynamicReceiver` means the *receiver itself* resolved to `Dynamic` — not "the checker gave up for any reason on a message send." Early inference code reused `DynamicReceiver` as a catch-all whenever a `Known`-typed receiver's method existed but declared no return type (e.g. `Boolean>>ifTrue:`/`ifFalse:`, which deliberately omit `-> R` — see BT-2834), even though the receiver was perfectly well-typed. That mislabeling both showed a misleading "(dynamic receiver)" reason on a well-typed receiver in `type-coverage --detail`, and suppressed the BT-1914 "Dynamic in typed class" warning (which skips `DynamicReceiver` on the assumption it was already reported at the receiver). BT-2868 fixed the `ifTrue:`/`ifFalse:` case at its root by inferring `Boolean | R` instead of collapsing to `Dynamic` at all, and reclassified the remaining unavoidable fallback for that selector family as `UnannotatedReturn` (already an existing reason — "method has no return type annotation and body could not be inferred") so the honest reason surfaces and the BT-1914 warning fires. Other pre-existing methods with an intentionally-undeclared return type (e.g. `Block>>on:do:`) still report `DynamicReceiver` for their own terminal fallback — narrowing those is unaddressed follow-up work, not part of BT-2868's scope.
