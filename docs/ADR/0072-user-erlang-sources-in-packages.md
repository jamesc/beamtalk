...
**IDE edit → compile → reload → write-back** (BT-2670, LiveView IDE): a **project-owned** native (`.erl`) module — one under the project's own `native/`, not a dependency or stdlib — is editable in the System Browser's native (`:native`) editor tab. Saving the buffer (`save-native-source` op) compiles the edited source, hot-reloads the module into the live VM, and writes the source back to disk. This is the editor surface over the same `compile:file/2` chokepoint Project load / single-file reload use (Section 9); it adds no new compilation path.

Native edits are **direct compile + write-to-disk**, not deferred ChangeLog entries. The method/class ChangeLog (ADR 0082) models *Beamtalk* method/class patches against the live image, with a flush step that reconciles image→disk. Native modules have no per-method install chokepoint and no per-method ChangeLog provenance — they are whole-`.erl` units compiled by `erlc` — so a deferred/flush model would add a parallel, ChangeLog-shaped bookkeeping layer for no benefit. Instead, a successful compile writes the `.erl` immediately (atomic temp-file + rename), so image and disk stay coherent in one step. The write is **fail-safe**: the edited buffer is compiled against a temporary `.erl` first, so a compile error returns structured diagnostics (the same `format_erl_compile_errors/2` maps the load path produces) and **leaves the on-disk source untouched**.

**Editability is gated on origin, re-derived server-side.** Only `source_origin = project` natives are writable; dependency and stdlib natives are read-only. The IDE surfaces an `editable` flag (a UX hint), but the `save-native-source` op never trusts it or any client-supplied path: it re-resolves the module's on-disk `.erl` from the module's own compile info (`module_info(compile)` → `source`) and re-classifies its origin (`beamtalk_repl_ops_browse:native_module_editable_target/1`), rejecting any deps/stdlib/outside-project target with a structured `#beamtalk_error{}`. The op is Owner-gated (`:execute` capability) on top of that guard. Editing dependency or stdlib native source is intentionally out of scope — dependencies are immutable inputs (Section 5), edited upstream and re-resolved, not patched in place.
