...
rebar3 handles include paths, code paths, compilation order, and incremental rebuilds automatically.

### 5. Native Dependency Resolution

Libraries declare version **constraints** in `[native.dependencies]`. Only the root application resolves them — matching how hex.pm, rebar3, and Mix all work.
...
**Solution: Top-level aggregation.**

`beamtalk build` walks the entire Beamtalk dependency graph, collects all `[native.dependencies]` constraints from every transitive package, and merges them into a single `rebar.config`:

```erlang
%% Merged from: http (gun ~> 2.1, cowboy ~> 2.12), websocket (gun ~> 2.0, cowboy ~> 2.12)
{deps, [
    {gun, "2.1.3"},       %% satisfies both ~> 2.1 and ~> 2.0
    {cowboy, "2.12.0"}    %% identical constraint, trivially merged
]}.
```
...
2. Merges constraints — if two packages declare the same hex dep, constraints must be compatible
3. On first build (no lockfile): runs rebar3 to resolve constraints, captures resolved versions in `beamtalk.lock`
4. On subsequent builds: generates `rebar.config` with exact pinned versions from `beamtalk.lock`
