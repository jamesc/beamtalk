...
- **BEAM veteran**: "Native `gen_server` dispatch has no schema, no versioning, and no contract enforcement. The `{Selector, Args, FuturePid}` tuple is an implicit convention — nothing prevents a caller from sending `{increment, [], wrong_pid}` and getting a silent failure. A serialization boundary makes the contract explicit: the protocol definition *is* the schema, malformed messages are rejected at the boundary, and you can version the format independently of the implementation. When actors eventually span nodes (distributed Erlang), you'll be doing serialization anyway — might as well design for it now."
