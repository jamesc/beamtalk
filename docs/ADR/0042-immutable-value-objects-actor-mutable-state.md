...
| Cohort | Their strongest argument |
|--------|------------------------|
| **Smalltalk purist** | "This is the only alternative that actually preserves Smalltalk's object model. Every object has identity, state, and behavior. Alan Kay said it's about the messages — and this is the only design where every message is actually a message." — **Philosophically correct, practically catastrophic.** A `#(1, 2, 3) collect: [:x | x + 1]` creates 3 Integer processes, sends `+` as a gen_server:call to each, creates 3 new Integer processes for the results, and wraps them in a List process. That's 7+ processes and 6+ context switches for what should be a microsecond operation. The Smalltalk purist argument assumes message-send cost is uniform — on BEAM, process-mediated sends are ~1000x more expensive than function calls. |
...
- **Surfaces BEAM's fundamental split at the right moment.** Every BEAM developer must eventually learn the difference between processes (identity, lifecycle, message-passing) and terms (data, copied between processes, no overhead). On raw BEAM/Erlang, this is a runtime lesson — learned through performance cliffs, mysterious bugs, or reading OTP docs. Beamtalk turns it into a **declaration-time choice**: `Actor subclass:` vs `Value subclass:`. The developer learns the concept when defining their class, not when production is on fire. The compiler knows which they chose and can guide them — `self.slot :=` works in actors, `with*:` works everywhere, and error messages explain why. This is Beamtalk's special sauce: making BEAM's process/term distinction accessible and explicit, rather than implicit and surprising.
...
- **Two state models.** Value types prohibit `self.slot :=` while actors allow it. This is a genuine semantic split — in Smalltalk, all objects work the same way. Developers must know whether they're in a Value or Actor class before knowing whether slot assignment works. However, this maps directly to a distinction BEAM developers must learn anyway (process vs term), and learning it at class declaration time is strictly better than learning it through runtime failures. The `Value`/`Actor` superclass makes the choice visible and the compiler enforces it.
