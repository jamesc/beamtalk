...
**Counter:** Valid performance concern, but the gen_server overhead (~5μs per call) is negligible compared to actual I/O latency (~100μs+ per disk read). The real value of the actor model isn't concurrency — it's **OTP supervision for resource management**: (1) if your code crashes mid-read, the supervisor terminates the FileStream, triggering `terminate/2` which closes the handle — no leaked file descriptors, (2) `Workspace actors` can list open files for inspection ("why is my disk full?"), (3) workspace shutdown cleanly closes all FileStreams via the supervision tree. This is "let it crash" applied to resource cleanup — idiomatic OTP. A value type can't provide this without manual `ensure:` blocks that users will forget.
