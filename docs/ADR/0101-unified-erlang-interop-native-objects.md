...
**`native:` must NOT lower to a bare `mod:Fn(Self, Args)` call.** A bare call bypasses the FFI proxy and would therefore get *neither* wrap-by-default (Part 2) *nor* the ok/error→`Result` coercion (ADR 0076) — re-introducing the exact "two FFI behaviors" inconsistency this ADR removes (e.g. `File readAll:` would return a raw `{ok, Bin}` instead of `Result(String, Error)`). Instead it lowers through a shared helper `beamtalk_erlang_proxy:native_call(Mod, Fn, Args, {Class, Sel})` that applies the **same** coercion + wrapping as inline `(Erlang …)` FFI, but **skips the dynamic export/arity checks** `direct_call/3` performs (codegen already guarantees the function and arity exist). Because codegen passes the Beamtalk `{Class, Sel}`, `native:` errors carry `Stream>>take:` context — better than the bare-MFA context raw inline FFI gets (see Part 2 limitation). The helper is also the single seam for any future proven-safe fast path, should profiling ever justify one.
...
**Performance:** wrapping adds a `try/catch` frame per FFI/`native:` call (`ensure_wrapped` runs only on the error path, never on success). For inline FFI this is no change — the proxy is already wrapped today. The frame is a constant amortized against coarse-grained FFI work (one call = an I/O op, ETS access, or an in-Erlang fold), and the hot fine-grained path (`@primitive`/`@intrinsic`) bypasses the proxy entirely. The only behavioral change is that `try/catch` defeats tail-call optimization on the wrapped call — a non-issue for `native:` methods, which bottom out in Erlang that loops internally. No fast-path carve-out in v1; the `native_call` helper is the seam to add one if profiling ever justifies it.
