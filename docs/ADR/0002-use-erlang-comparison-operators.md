...
1. **The BEAM would not honour it.** `Dictionary` is backed by Erlang maps and `Set` by map keys, so key identity is the VM's own `=:=` — as are `lists:member/2`, `ets`, and receive-pattern matching. None can be taught about a Beamtalk override. Dispatch would buy `a =:= b` → `true` while a `Set` holding both still reported size 2: a silent contradiction, strictly worse than today's honest limitation, where the operator agrees with every container.
