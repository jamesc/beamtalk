...
1. **The BEAM would not honour it.** The keyed containers decide identity in the VM, below any Beamtalk method. `Dictionary` is backed by Erlang maps, so keys compare with `=:=`. `Set` is backed by `ordsets`, so elements compare by Erlang term *order*, which is `==` semantics — `Set new add: 1; add: 1.0` holds one element, where a `Dictionary` keyed on `1` and `1.0` holds two. (That divergence predates this ADR and is tracked separately; it is noted here because it shows how far identity in these containers is from anything the language controls.) The same applies to `lists:member/2`, `ets`, and receive-pattern matching. None can be taught about a Beamtalk override, so dispatch would buy `a =:= b` → `true` while a `Set` holding both still reported size 2 — a silent contradiction, strictly worse than the honest limitation.
