...
- The existing `with_live_supervisor/3` helper in `beamtalk_supervisor.erl` already uniforms stale-handle errors into `beamtalk_error` — that machinery stays; we swap the `error/1` raise for a `Result error:` construction.
...
1. **Keep `startLink/1` raising; wrap at the stdlib layer with `Result tryDo:`.** No runtime change; `supervise` body becomes `Result tryDo: [(Erlang beamtalk_supervisor) startLink: self]`. The block returns the bare `{beamtalk_supervisor_new, ...}` tuple, class_dispatch hook runs before `tryDo` wraps, initialize runs in caller's process, and `tryDo` sees the final (already-normalized) supervisor tuple. **Concern:** the Error side becomes an Exception-wrapped value, not a structured `Result error: (beamtalk_error supervisor_start_failed)` matching the ADR's contract. The Exception does carry class/selector/message from `ensure_wrapped`, so the difference is at the type signature (`Error<Exception>` vs `Error<#beamtalk_error{}>`) more than the user-visible content.
2. **Teach the class_dispatch hook to inspect the `Result` tagged map.** The hook pattern-matches a second case for `Result ok: {beamtalk_supervisor_new, ...}`, rewrites the inner tag, runs initialize, and returns `Result ok: {beamtalk_supervisor, ...}`. **Concern:** couples `beamtalk_class_dispatch` to Result's internal representation; leaks supervisor-specific logic into a generic dispatch layer.
3. **Run initialize: synchronously from `startLink/1` via a short-lived helper process.** Spawns a transient process to run initialize before returning; caller waits for it. **Concern:** changes the documented "runs in caller's process" guarantee — downstream implications unknown; probably needs its own mini-ADR.

Probe 0a should compare options (1) and (2) empirically: the probe commit implements both for a single method (e.g., `terminate:`) and measures which keeps the e2e supervisor test suite green with the least collateral code.
...
- **`runtime/apps/beamtalk_runtime/src/beamtalk_supervisor.erl`:**
  - `startLink/1` — implementation driven by Probe 0a. If option 1 wins, `startLink/1` is unchanged and the Result conversion happens in Phase 2's stdlib body. If option 2 wins, `startLink/1` returns `{ok, SupTuple}` / `{error, BtError}` and the class_dispatch hook is extended.
