...
**Why deferred, not rejected**: Option C's fast path is Option A — the `method_return_types` chain walk — verbatim. The fallback adds port-based inference for unannotated methods on top. This means Option A is the correct first step: it builds exactly the infrastructure Option C needs, without committing to the port fallback before the ClassHierarchy-in-port problem is solved. When that problem is resolved (stateful compilation session, richer port protocol, or another mechanism), layering the Option C fallback onto the Option A foundation is a small additive change. Option C is not rejected — it is the natural upgrade path from Option A.
