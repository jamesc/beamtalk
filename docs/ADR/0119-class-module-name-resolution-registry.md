...
- Should `beamtalk_repl_ops_load.erl`'s regex-based `build_source_class_module_index/1` be replaced in this same epic (routing workspace/REPL index construction through the compiler-port into the new Rust registry), or tracked as an explicit follow-up issue? Left as-is, this ADR's "single source of truth" claim does not hold for workspace-mode sessions.
