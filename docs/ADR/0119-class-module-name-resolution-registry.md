...
- **No behavior change**: this is a refactor. Compiled output for the existing stdlib/example/test corpus must be byte-identical. (An earlier draft of this ADR anticipated `Future`'s resolution as a likely exception; verified it is not — `Future` is never actually resolved through `compiled_module_name` on any live path today, see below — so no exception is expected. The corpus check remains how we'd find out if that assumption is wrong.)
...
6. Regression tests reproducing each of BT-3081/BT-3431/BT-3432's specific scenarios against the unified registry (per the issue's acceptance criteria), plus a subdirectory-dispatch test. (No `Future`-specific regression test — nothing resolves its module on a live path today; see Decision.)
