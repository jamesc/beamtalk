...
| `renameTo:` | **Refuse** | Allowed, `flushable: false` (`"dynamic"`) | **Refuse** | Renaming a class's *identity* is not "patch one method for live debugging" — it corrupts the class's own cross-references (extension attribution, xref implementor rows) for a class you don't author. A dynamic class has no such external referents to break (ADR 0038); it is the caller's own construction. |
| `renameSelector:to:` | Allowed, `flushable: false` (`"stdlib"`) | Allowed, `flushable: false` (`"dynamic"`) | Allowed, `flushable: false` (`"dependency:<path>"`) | Same granularity argument ADR 0112 already made for `removeSelector:` — a single-selector operation, not a whole-class identity change; `Integer renameSelector: #printString to: #displayString` is exactly as legitimate a live-debugging move as `Integer compile: #printString source: ...` already is today. |
...
{ts, seq, epoch, class: "<new name>", selector: null,
 ...
 source_ref: null, prev_source_ref: null,        % no body changes — a pure move (+ declaration-line rewrite, see Implementation)
 sourceFile: "<old_path>",                        % what flush's external-edit check watches
 span: null,
 intent: "durable",
 flushable: bool,
...
`rename-method` is the one genuinely new shape — every other kind ADR 0082/0112 defined targets exactly one file. A rename's `flushable` is `true` only if **every** site (definition + all current senders) resolves to a flushable file; if even one sender lives in a dependency or stdlib file, the whole rename entry is `flushable: false` with that reason, because a rename that could only partially reach disk (definition renamed, some senders left calling the old name) is worse than not flushing at all — it would silently split the live and on-disk selector surfaces for those files.
...
| Class rename | Write `<new_path>.tmp` (declaration-line rewritten to the new name; rest of the file byte-identical) | Rename `<new_path>.tmp` → `<new_path>`, then `unlink <old_path>` |
...
| Target file for a `remove-class`/`rename-class` was already deleted externally | `stat` fails at Phase A | Surfaces as `already gone — nothing to remove` (for `remove-class`, a soft success: the entry is pruned, the outcome the user wanted already holds) or `source file relocated or deleted` (for `rename-class`, ADR 0082's existing relocation conflict kind, unchanged) |
...
| A `rename-method` sender site's span no longer resolves (the sender was independently edited since the rename was computed) | Existing byte-span resolution, per-site | That one site fails Phase A, aborting the whole `rename-method` entry (not a partial rename — see *Consequences*) |
...
**Adopted:** the "compute every affected site before touching anything, apply as one transaction" shape for `renameSelector:to:` — this is exactly the multi-site `sites` list and Phase A validate-everything-before-writing-anything design above.
...
- The Refactoring-Browser-style "compute every sender, apply together" behaviour of `renameSelector:to:` is the single most Smalltalk-native piece of this ADR — a Pharo developer would be surprised if it worked any *other* way.
...
### Alternative: block `renameSelector:to:` on non-empty senders outside the current in-project rename batch (never leave a partial rename)

Considered for the case where a rename's `sites` list, computed via the xref index, might miss a sender that exists in a file the xref index hasn't indexed yet (a freshly-created, not-yet-compiled file) or a dynamically-constructed send (`aClass perform: (aString asSymbol))`) that no static xref can see. Rejected as a hard block: ADR 0112 already accepted this exact risk category for `removeSelector:` ("dangling senders... not prevented, but the tooling to surface them already exists") and gave a considered reason not to block on it — the same reasoning applies here without a new argument. `renameSelector:to:` instead surfaces the sender count found (mirroring ADR 0112's hint text) so the caller has visibility, without refusing an action ADR 0112 already decided is the caller's call to make.
...
- **`renameSelector:to:` is the most implementation-heavy primitive this codebase has added via an ADR to date** — it requires recompiling not just the target method (as removal already does) but every current sender, computed transactionally, with a `sites` list that has no precedent in the existing schema shape (every prior kind targets exactly one file). This is real complexity, not a documentation-only extension.
- **A rename can only ever rewrite senders the xref index can see.** Dynamically constructed sends (`perform:`, string-built selectors) and senders in files that haven't been compiled/indexed yet are invisible to `sites` and become dangling exactly the way ADR 0112 already accepted for `removeSelector:` — this ADR does not raise that bar, but a rename's blast radius (across N files) makes an invisible dangling sender more likely to matter than a single-selector removal's.
...
- **Compilation context** gains the multi-site rewrite step `renameSelector:to:` needs (rewrite N spans across N files, each independently re-parsed and re-validated) — an extension of the existing byte-span resolver (ADR 0082), not a new subsystem.
...
- **Runtime context** gains `classRenameTo`/`classRenameSelector`/`classRenameSelectorIfAbsent` primitives alongside the existing `classRemoveSelector*` family in `beamtalk_behaviour_intrinsics.erl`, and the ChangeLog-append call `removeFromSystem` is currently missing.
...
| `runtime/apps/beamtalk_workspace/src/beamtalk_repl_eval.erl` / `beamtalk_repl_loader.erl` | `renameSelector:to:`'s sender-rewrite step generalizes the same recompile-based mechanism ADR 0112's Implementation section already generalized for `removeSelector:` (`remove_method/3` → a sibling `rename_method_sites/4` or equivalent) — reuse, not a parallel mechanism, per CLAUDE.md's no-duplicate-implementations rule. Needs the xref `sendersOf:` query (already exists, used today for ADR 0112's dangling-sender hint) wired in as the site-discovery step. |
| `runtime/apps/beamtalk_workspace/src/beamtalk_workspace_changelog.erl` | New `kind`s (`'remove-class'`, `'rename-class'`, `'rename-method'`), new fields (`old_class`, `old_path`, `new_path`, `sites`) on the entry record, extending the existing open-schema pattern. `target_key/1` (ADR 0112, BT-3187's `(class, selector, side)` fix) needs a variant for `rename-method`'s multi-site shape — likely keying shadow-detection per-site rather than per-entry, since two independent renames could touch overlapping sender files. |
...
| `stdlib/src/Workspace.bt` | `flush: confirmDestructive:` keyword variant on the existing `flush`/`flush:` facade methods; `flushKinds:confirmDestructive:` on `ChangeLog`; `moveClass:to:` for the path-only-move case (no identity change) that `rename-class` also covers. |
...
| **3** | `renameTo:` + class-rename flush (file move, declaration-line rewrite, `Workspace moveClass:to:` for the path-only case). | M | EUnit: rename-to-tmp-then-final-rename atomicity; BUnit: `Counter renameTo: #Accumulator` + flush produces the moved file with an updated declaration. |
| **4** | `renameSelector:to:`/`ifAbsent:` — the multi-site mechanism: xref-driven sender discovery, transactional in-memory rewrite of definition + senders, `sites`-shaped ChangeLog entry, multi-file flush. Largest phase; depends on Phase 2's tiering machinery. | L | EUnit: sender rewrite correctness against a small fixture graph (class with 3 senders across 2 files); BUnit: end-to-end rename + flush + verify no dangling `does_not_understand`. |
...
Total: ~M-L across 6 phases. Phase 4 (`renameSelector:to:`) is the load-bearing risk — if the sender-rewrite mechanism proves unreliable against real code (partial rewrites, xref gaps), the design may need to fall back to a narrower v1 (rename the definition only, leave senders as a `Workspace changes` follow-up list for the caller to apply manually) before committing to the full transactional shape.
