...
| Method rename | Rewrite the target file's span **and every confirmed sender's span** (`self_recv`/`super_recv` sites — see Decision; `other` senders are reported as `candidate_sites`, never rewritten) | Yes, but N files are touched that the caller didn't directly ask to edit | 2 — `confirmDestructive` |
...
{ts, seq, epoch, class, selector: "<new selector>", old_selector: "<old selector>",
 ...
 candidate_sites: [{sourceFile, span: {start, end}}, ...],
   %% `other`/`erlang_ffi` sends found via the same sendersOf: query —
