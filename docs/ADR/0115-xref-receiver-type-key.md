...
**Epic:** BT-2798
**Issues:** BT-3216 (Phase 1 spike) → BT-3217 (Phase 2 schema/write path) → BT-3218 (Phase 3 read path) → BT-3219 (Phase 4 recheck integration) → BT-3220 (Phase 5 benchmarks/e2e/docs), sequentially blocked in that order and all merged. BT-3215 (precise `Union`/`Intersection` keying) tracked separately, deliberately out of this epic's scope.
**Status:** Implemented. Phase 5 (BT-3220) re-ran `bench_senders_xref.escript`/`bench_recheck_fanout.escript` against the shipped mechanism and confirmed both halves of this ADR's own success criteria: no regression to `senders_of/1` (Constraint-preserving, additive `recv_type`), and BT-2781's synthetic 90%-loss scenario measurably fixed for typed candidates (0% loss at the same 10x-cap fan-out). An E2E REPL-protocol test (`tests/repl-protocol/cases/adr_0115_recv_type_recheck_fanout.btscript`) proves the mechanism is reachable from the REPL surface, not just correct at the EUnit level. Full numbers: `docs/development/benchmarks.md` § "Update — `senders_of/2` implemented and measured".
