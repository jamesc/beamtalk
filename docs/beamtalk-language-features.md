...
Most reflection methods are implemented in the `dispatch/4` function, after user-defined methods but before the `doesNotUnderstand:args:` fallback. The `perform:` and `perform:withArguments:` methods are handled in the frontend code generator (`builtins.rs`) by sending the target selector directly to the actor mailbox.
