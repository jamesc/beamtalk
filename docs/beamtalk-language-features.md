...
Reflection methods are declared in the stdlib via pragma annotations (see [ADR 0007](ADR/0007-compilable-stdlib-with-primitive-injection.md)). For example, `respondsTo:` and `instVarNames` are declared in `lib/Object.bt` with `@primitive respondsTo` and `@primitive instVarNames` pragmas that route through the runtime dispatch modules (`beamtalk_primitive.erl` → type-specific modules). The `perform:` and `perform:withArguments:` methods are declared in `lib/ProtoObject.bt` with `@primitive dynamicSend`, which generates code to send the target selector directly to the actor mailbox.
