...
| `live-edit-method` | -- | *(via `eval`: `Class >> sel => body` / `Class class >> sel => body`, or `Class compile: #sel source: body`)* | *(via `evaluate`; durable `save_method` ≡ `compile:source:`, ephemeral `try_method` ≡ `tryCompile:source:`)* | -- | Live-patch an instance- or class-side method on a registered class (ADR 0066 / ADR 0082 Phase 1 / ADR 0084). The `>>` patcher form desugars to `compile:source:` (durable); `tryCompile:source:` is the ephemeral variant; both take the body as a String value. Every successful in-memory patch emits a ChangeLog entry. Reached through the shared `eval`/`evaluate` surface; the class-side path recompiles the class's recorded source. No dedicated op. |
| `new-class` | -- | *(via `eval`: `Workspace newClass: source at: path`)* | *(via `evaluate`; future `save_class` ≡ `newClass:at:`, ADR 0082 Phase 3)* | *(future `executeCommand: save_class`, ADR 0082 Phase 3)* | Create a brand-new class from a source String at a target path (ADR 0082 Phase 1). Compiles and installs the class in memory and logs a durable `kind: "new-class"` ChangeLog entry; Phase 1 does not write the file (that is Phase 2 flush). Raises a loud, specific error (no silent fallback) when the target already exists, lies outside the project tree, the declared class name does not match the path basename, or a class of that name is already loaded. Reached through the shared `eval`/`evaluate` surface; no dedicated op. |
| `flush` | -- | `:flush` / `:flush <selector>` ≡ `Workspace flush` / `Workspace flush: <selector>` | *(via `evaluate`; future MCP `flush` ≡ `Workspace flush`, ADR 0082 Phase 3)* | *(future `executeCommand: flush`, ADR 0082 Phase 3)* | Write the pending ChangeLog entries to disk (ADR 0082 Phase 2). Splices each durable+flushable entry's body back into its source file via byte-span replacement (no AST reprint), atomically (`<file>.tmp` + atomic rename), with external-edit conflict detection. Multi-file flushes use a two-phase commit: write every `<file>.tmp` first, then rename each in sequence; any Phase A failure aborts the whole flush with no rename happening. Returns a `FlushResult` summary recording what was written (`flushed`, `files`, `newClasses`) and what conflicted (`conflicts`, with `reason ∈ {external_edit, target_exists, span_out_of_range, source_file_unreadable, rename_failed}`). Flushed entries stay in the audit log but drop out of the active `Workspace changes` view. The REPL `:flush` meta-command (ADR 0082 Phase 3, BT-2287) is a CLI-side shortcut that constructs the equivalent Beamtalk expression — no dedicated workspace-side op. `<selector>` may be a Class (`:flush Counter`), a Symbol kind (`:flush #'new-class'`), or a Dictionary (`:flush #{ #file => "src/foo.bt" }`). |
| `changes` | -- | `:changes` ≡ `Workspace changes` | *(via `evaluate`; future MCP `list_changes` ≡ `Workspace changes`, ADR 0082 Phase 3)* | -- | Return the workspace ChangeLog — the navigable view of pending in-memory changes (ADR 0082, BT-2284). Backed by `beamtalk_workspace_changelog:changeLog/0` via FFI. The REPL `:changes` meta-command (ADR 0082 Phase 3, BT-2287) is a CLI-side shortcut for the underlying message send. All pending-state queries (`size`, `notEmpty`, `dirtyMethods`, `select:`, ...) live on the returned ChangeLog object per Pharo's `Smalltalk changes` idiom — no dedicated workspace-side op. |
| `dirty` | -- | `:dirty` ≡ `Workspace changes notEmpty` | *(via `evaluate`; future MCP `dirty` ≡ `Workspace changes notEmpty`, ADR 0082 Phase 3)* | -- | Answer "is anything pending?" (ADR 0082, BT-2284). Returns a Boolean. The REPL `:dirty` meta-command (ADR 0082 Phase 3, BT-2287) is a CLI-side shortcut that composes from the existing `changes` primitive — no dedicated workspace-side op. Equivalent to `Workspace changes notEmpty` (or `(Workspace changes isEmpty) not`). |
