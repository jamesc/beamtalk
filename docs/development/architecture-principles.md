...
|---|---|---|
| Two implementations in different languages, compiled/reviewed separately, that must agree on wire format or behavior | **Keep** — this is the permanent boundary the test exists to guard | `beamtalk-surface-drift` (CLI/REPL/MCP/LSP), `beamtalk-parity-tests` (cross-process parity), the Rust↔Erlang conformance fixtures from BT-3080/BT-3081/BT-3085/BT-3090, BT-3542 (`sendability.rs`'s kind-based tier fallback vs. `beamtalk_shape_migration:field_tier/1`'s `pack/1` walk — both pinned to `runtime/apps/beamtalk_runtime/test/fixtures/sendability_tier_conformance.json`), and BT-3556 (ADR 0124 §8/B9: `reconcile_declared/7`'s `(slot kind, key present?, has default?, typed?) -> outcome` decision table vs. this crate's compiled `field_kinds`/`field_has_default`/`is_typed` meta emission — pinned to `runtime/apps/beamtalk_runtime/test/fixtures/late_reconcile_conformance.json`, asserted by `late_reconcile_conformance.rs` and `beamtalk_shape_migration_tests:test_late_reconcile_conformance_matches_shared_corpus/0`) |
