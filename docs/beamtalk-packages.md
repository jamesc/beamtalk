...
Keys are kebab-case diagnostic categories (`dnu`, `type`, `unused`, `empty-body`, `lint`, `dead-assignment`, `extension-conflict`, `deprecation`, `actor-new`, `visibility`, `unresolved-class`, `unresolved-ffi`, `arity-mismatch`, `shadowed-class`, `type-annotation`, `inheritance`, `sendability`, `native-declaration-location`, `file-class-name-mismatch`, `definite-assignment`, `unguarded-late-read`). Values are one of `"off"` (drop the diagnostic entirely), `"lint"`, `"hint"`, `"warn"`, or `"error"` (fails the build unconditionally, independent of `--warnings-as-errors`).
...
`unguarded-late-read` ([ADR 0124](ADR/0124-slots-late-assignment-definite-assignment.md) §4d) warns on a direct `self.slot` read of a `late` slot reachable from `terminate:`/`handleInfo:` (that method itself, or one level of `self`-send it makes within the same class) that isn't guarded by a `(self hasField: #slot) ifTrue: [...]` test on the same slot — see [`late` Slots](beamtalk-language-features.md#late-slots-adr-0124) in the language reference. Both lifecycle hooks swallow a raised error (`terminate/2`'s dispatch is `try`-wrapped; `handleInfo:` logs and continues), so an unguarded read there raises `UninitializedStateError` silently instead of crashing loudly. Like `definite-assignment`, it has no site-level `@expect` category by design — the exemption is the `hasField:` guard, not an annotation.
