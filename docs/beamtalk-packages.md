...
`BEAMTALK_REGISTRY_CACHE_DIR` overrides the cache *root* — a hash of the registry URL is still appended underneath it (same as the default location), so two different registry URLs pointed at the same override never clobber each other's clone. Point it at `_build/registry` for a per-project cache close to the pre-BT-2996 layout, or at a team-wide mount to share a pre-warmed clone across machines. A stale or corrupt cache is cleared by deleting its directory (or `~/.beamtalk/registry/` entirely, or `_build/registry/` via `beamtalk clean --deps` when the override is in effect) — it is rebuilt from a fresh clone on the next lookup.
