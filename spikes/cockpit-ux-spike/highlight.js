/* highlight.js — Beamtalk syntax highlighter (Smalltalk-INSPIRED grammar).
   window.highlightBeamtalk(src) -> HTML with span.tok-* wrappers.
   Beamtalk specifics vs classic Smalltalk:
     - comments are // line and slash-star block (NOT double-quoted)
...
(function () {
  ...
  function hlString(text) {
    ...
    while (i < text.length) {
      if (text[i] === "{") {
        ...
        if (end > i) {
          out += '<span class="tok-interp">' + esc(text.slice(i, end + 1)) + "</span>";
          i = end + 1;
          continue;
        }
      }
  ...
  window.highlightBeamtalk = function (src) {
    if (src == null) return "";
    let i = 0, out = "";
    const n = src.length;
    while (i < n) {
      ...
      for (const rule of RULES) {
        ...
        else if (rule.cls === "field") out += '<span class="tok-punct">.</span>' + span("field", text.slice(1));
