...
%%% @doc Tests for WorkspaceInterface singleton (Phase 1 gen_server — DEPRECATED).
%%%
%%% @deprecated beamtalk_workspace_interface (Phase 1 gen_server) is planned for removal
%%% per BT-1093. New tests should target beamtalk_workspace_interface_primitives_tests instead.
%%%
...
has_method_returns_false_for_unknown_selectors_test() ->
    ...
    ?assertNot(beamtalk_workspace_interface:has_method(version)),
    ?assertNot(beamtalk_workspace_interface:has_method('doesNotExist:')).
...
class_info_returns_valid_metadata_test() ->
    ...
    ?assertEqual('WorkspaceInterface', maps:get(name, Info)),
    ?assertEqual(beamtalk_workspace_interface, maps:get(module, Info)),
...
named_terminate_cleans_registration_test() ->
    {ok, Pid} = beamtalk_workspace_interface:start_link({local, 'Workspace'}),
