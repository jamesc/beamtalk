...
%% and `row_doc_signature/4` (the BT-2735/BT-2714 synthetic-only doc/signature
%% resolver) are the same per-selector shaping op 2 (`browse-protocols`) already
%% does. Exported so the `methods` ws op (`beamtalk_repl_ops_dev`) — the VS Code
%% Workspace Explorer sidebar's method-list channel — can badge/hover a
...
-export([info_fields/1, row_doc_signature/4]).
