...
%%% - Actor migration (migrate:/migrate:with:)
%%% - Error cases
...
%%% BT-106: migrate: protocol tests

migrate_via_dispatch_sync_test() ->
    %% Test migration through the full dispatch path (sync call)
    {ok, Counter} = test_counter:start(42),
    ?assertEqual(42, gen_server:call(Counter, {getValue, []})),

    %% Send migrate: through gen_server:call
    Result = gen_server:call(Counter, {'migrate:', [test_counter_v2]}),
    ?assertMatch({ok, #beamtalk_object{class = 'CounterV2'}}, Result),

    {ok, NewRef} = Result,
    NewPid = NewRef#beamtalk_object.pid,
    ?assert(is_process_alive(NewPid)),

    %% Value should be preserved from old actor
    ?assertEqual(42, gen_server:call(NewPid, {getValue, []})),

    %% Old actor should be stopped
    ?assertEqual(false, is_process_alive(Counter)),

    gen_server:stop(NewPid).

migrate_via_dispatch_async_test() ->
    %% Test migration through the async dispatch path (cast with future)
    {ok, Counter} = test_counter:start(100),
    ?assertEqual(100, gen_server:call(Counter, {getValue, []})),

    %% Send migrate: through async path
    Future = beamtalk_future:new(),
    gen_server:cast(Counter, {'migrate:', [test_counter_v2], Future}),
    NewRef = beamtalk_future:await(Future),

    ?assertMatch(#beamtalk_object{class = 'CounterV2'}, NewRef),

    NewPid = NewRef#beamtalk_object.pid,
    ?assert(is_process_alive(NewPid)),

    %% Value should be preserved
    ?assertEqual(100, gen_server:call(NewPid, {getValue, []})),
...
migrate_with_state_transformer_test() ->
    %% Migrate with a state transformation block
    {ok, Counter} = test_counter:start(50),
    ?assertEqual(50, gen_server:call(Counter, {getValue, []})),

    %% Migrate with transformer that adds a label
    Transformer = fun(UserState) ->
        UserState#{label => <<"migrated">>}
    end,
    Result = gen_server:call(Counter, {'migrate:with:', [test_counter_v2, Transformer]}),
    ?assertMatch({ok, #beamtalk_object{class = 'CounterV2'}}, Result),

    {ok, NewRef} = Result,
    NewPid = NewRef#beamtalk_object.pid,

    %% Value preserved, label set by transformer
    ?assertEqual(50, gen_server:call(NewPid, {getValue, []})),
    ?assertEqual(<<"migrated">>, gen_server:call(NewPid, {getLabel, []})),

    ?assertEqual(false, is_process_alive(Counter)),
    gen_server:stop(NewPid).

migrate_with_invalid_class_test() ->
    %% migrate: with a non-atom argument should fail
    {ok, Counter} = test_counter:start(0),
    Result = gen_server:call(Counter, {'migrate:', ["not_an_atom"]}),
    ?assertMatch({error, #beamtalk_error{kind = type_error}}, Result),
    %% Actor should still be alive after a failed migration
    ?assert(is_process_alive(Counter)),
    gen_server:stop(Counter).

migrate_with_invalid_transformer_test() ->
    %% migrate:with: with a non-function transformer should fail
    {ok, Counter} = test_counter:start(0),
    Result = gen_server:call(Counter, {'migrate:with:', [test_counter_v2, not_a_function]}),
    ?assertMatch({error, #beamtalk_error{kind = type_error}}, Result),
    ?assert(is_process_alive(Counter)),
    gen_server:stop(Counter).

migrate_with_failing_transformer_test() ->
    %% migrate:with: where the transformer raises an error
    {ok, Counter} = test_counter:start(0),
    BadTransformer = fun(_State) -> error(intentional_failure) end,
    Result = gen_server:call(Counter, {'migrate:with:', [test_counter_v2, BadTransformer]}),
    ?assertMatch({error, #beamtalk_error{kind = runtime_error}}, Result),
    ?assert(is_process_alive(Counter)),
    gen_server:stop(Counter).

migrate_with_non_map_transformer_result_test() ->
    %% migrate:with: where the transformer returns a non-map
    {ok, Counter} = test_counter:start(0),
    BadTransformer = fun(_State) -> not_a_map end,
    Result = gen_server:call(Counter, {'migrate:with:', [test_counter_v2, BadTransformer]}),
    ?assertMatch({error, #beamtalk_error{kind = type_error}}, Result),
    ?assert(is_process_alive(Counter)),
    gen_server:stop(Counter).

migrate_responds_to_test() ->
    %% respondsTo: should report true for migrate: and migrate:with:
    {ok, Counter} = test_counter:start(0),
    ?assertEqual(true, gen_server:call(Counter, {'respondsTo:', ['migrate:']})),
    ?assertEqual(true, gen_server:call(Counter, {'respondsTo:', ['migrate:with:']})),
    gen_server:stop(Counter).

migrate_nonexistent_module_test() ->
    %% migrate: to a module that doesn't exist should fail gracefully
    {ok, Counter} = test_counter:start(0),
    Result = gen_server:call(Counter, {'migrate:', [nonexistent_module_xyz]}),
    ?assertMatch({error, #beamtalk_error{kind = runtime_error}}, Result),
    ?assert(is_process_alive(Counter)),
    gen_server:stop(Counter).
