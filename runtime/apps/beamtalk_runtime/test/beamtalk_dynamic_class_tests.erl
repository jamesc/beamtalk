...
superclass_not_found_test() ->
    ...
    ?assertMatch(
        {error, #beamtalk_error{kind = class_not_found, class = 'NonExistentClass'}}, Result
    ).
...
repl_style_usage_test() ->
    ...
    ClassPid = element(
        2,
        beamtalk_object_class:create_subclass('Actor', 'REPLCounter', #{
            instance_variables => [count],
            instance_methods => #{
                increment => fun(_Self, [], State) ->
                    Count = maps:get(count, State, 0),
...
invalid_method_arity_test() ->
    ...
    Result = beamtalk_object_class:create_subclass('Actor', 'BadArity', #{
        ...
        instance_methods => #{
            % Arity 1, not 3
            badMethod => fun(State) -> {reply, ok, State} end
        }
    }),
