...
detect_found_test() ->
    ?assertEqual(4, beamtalk_list_ops:detect([1, 2, 3, 4, 5],
                                             fun(X) -> X > 3 end)).

detect_first_match_test() ->
    ?assertEqual(2, beamtalk_list_ops:detect([1, 2, 3, 4],
                                             fun(X) -> X rem 2 =:= 0 end)).
...
detect_if_none_found_test() ->
    ?assertEqual(4, beamtalk_list_ops:detect_if_none([1, 2, 3, 4, 5],
                                                      fun(X) -> X > 3 end,
                                                      default)).

detect_if_none_not_found_value_test() ->
    ?assertEqual(default, beamtalk_list_ops:detect_if_none([1, 2, 3],
                                                            fun(X) -> X > 10 end,
                                                            default)).

detect_if_none_not_found_block_test() ->
    ?assertEqual(42, beamtalk_list_ops:detect_if_none([1, 2, 3],
                                                       fun(X) -> X > 10 end,
                                                       fun() -> 42 end)).
...
do_iterates_test() ->
    ...
    ?assertEqual({Ref, 1}, receive {Ref, M1} -> {Ref, M1} after 100 -> timeout end),
    ?assertEqual({Ref, 2}, receive {Ref, M2} -> {Ref, M2} after 100 -> timeout end),
    ?assertEqual({Ref, 3}, receive {Ref, M3} -> {Ref, M3} after 100 -> timeout end).
...
reject_test() ->
    ?assertEqual([1, 3, 5],
                 beamtalk_list_ops:reject([1, 2, 3, 4, 5],
                                          fun(X) -> X rem 2 =:= 0 end)).

reject_none_test() ->
    ?assertEqual([1, 2, 3],
                 beamtalk_list_ops:reject([1, 2, 3],
                                          fun(X) -> X > 10 end)).
...
sort_with_ascending_test() ->
    ?assertEqual([1, 2, 3],
                 beamtalk_list_ops:sort_with([3, 1, 2],
                                             fun(A, B) -> A =< B end)).

sort_with_descending_test() ->
    ?assertEqual([3, 2, 1],
                 beamtalk_list_ops:sort_with([1, 3, 2],
                                             fun(A, B) -> A >= B end)).
...
zip_equal_test() ->
    ...
    ?assertEqual([#{<<"key">> => a, <<"value">> => 1},
                  #{<<"key">> => b, <<"value">> => 2}], Result).
...
group_by_test() ->
    Result = beamtalk_list_ops:group_by([1, 2, 3, 4, 5],
                                        fun(X) -> X rem 2 end),
...
each_with_index_test() ->
    Ref = make_ref(),
    Self = self(),
    beamtalk_list_ops:each_with_index([a, b, c],
                                       fun(Elem, Idx) -> Self ! {Ref, Elem, Idx} end),
    ?assertEqual({Ref, a, 1}, receive {Ref, E1, I1} -> {Ref, E1, I1} after 100 -> timeout end),
    ?assertEqual({Ref, b, 2}, receive {Ref, E2, I2} -> {Ref, E2, I2} after 100 -> timeout end),
    ?assertEqual({Ref, c, 3}, receive {Ref, E3, I3} -> {Ref, E3, I3} after 100 -> timeout end).
