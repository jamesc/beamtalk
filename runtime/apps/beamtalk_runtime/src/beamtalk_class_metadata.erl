...
`beamtalk_class_hierarchy`). They were consolidated because they
...
* `is_abstract` — whether the class is abstract, or `undefined` if unset
  (ADR 0109 amendment)
...
* Heir: created at app start by `beamtalk_runtime_app:start/2` (the
...
"seventh, independently-lifecycled table" that the original consolidation weighed against — this
...
%% Generated by `beamtalk build-stdlib` (see build_stdlib.rs's
...
-export([
    ...
    foldl_modules/2,
    all_builtins/0,
    %% ADR 0084: runtime class-method fun retrieval store + gate flag.
...
%% ADR 0084: sibling table holding runtime-installed class-method funs,
...
%% classes pay nothing on the dispatch hot path.
-define(FUN_TABLE, beamtalk_class_method_funs).
%% Reverse-edge index for match_subclasses/1 — a `bag` of
...
-record(class_metadata, {
    ...
    %% ADR 0084: true once a runtime/builder class-method fun has been
    ...
    %% ADR 0109 amendment: whether the class is abstract, read by the
...
once the supervisor is alive. Safe to call concurrently — a try/catch
...
new() ->
    ...
    %% The funs table is a plain {Key, Value} set (default keypos 1),
    ...
    %% Reverse-edge index, {Superclass, ClassName} rows, default
...
know `is_abstract` — ADR 0109 amendment).

Do NOT call this to update an **existing** row — besides wiping any
...
caller remembers a follow-up `seed_runtime_class_methods/2`. Use
`merge_identity/5` for updating an existing row instead; it never touches
the gate.
""".
-spec insert(
    class_name(),
...
insert(Name, Module, Selectors, Superclass, IsAbstract) ->
    ...
    new(),
    %% Read the row's previous superclass (undefined if there was no
...
Per-field update of `module`, `selectors`, `superclass`, and
...
merge_identity(Name, Module, Selectors, Superclass, IsAbstract) ->
    new(),
    %% Read the previous superclass before the update lands, so a
...
delete(Name) ->
    %% Drop any runtime class-method funs alongside the metadata row so
...
-doc """
Walk `ClassAtom`'s ancestor chain (via `lookup_superclass/1` and
`beamtalk_hierarchy:walk_ancestors/3`, the same depth-guarded walker every
hierarchy walk in this codebase is built on) and merge each ancestor's own
contribution — as `ReadOwnMapFun(AncestorClass)` reports it — into one map,
closer-to-`ClassAtom` levels winning a same-key conflict. Deliberately
**never** folds in `ClassAtom`'s own contribution — only its ancestors' —
so a caller that wants the flattened whole merges the result with its own
map itself (`maps:merge(AncestorMap, OwnMap)`).

This is the one ancestor walk-and-merge primitive every per-class-level-map
flattener in this codebase shares (CLAUDE.md's no-duplicate-implementations
rule): `beamtalk_workspace_shape_store`'s live, ETS-backed `field_types`/
`field_kinds` flattening and `beamtalk_release_shapes`'s build-time,
disk-derived flattening (ADR 0125 §2.2/§3.4) both call this with a different
`ReadOwnMapFun` (a live `__beamtalk_meta/0` read vs. a build-time one) rather
than each re-implementing the walk. `ReadOwnMapFun` is wrapped in a `try` —
an ancestor whose own map cannot be read (unregistered, no meta exported,
degraded to `#{}`) simply contributes nothing at its level rather than
failing the whole flatten, mirroring `beamtalk_workspace_shape_store`'s
original degrade for the identical case.

Degrades to the partial merge on a hierarchy cycle (`?MAX_HIERARCHY_DEPTH`
exceeded), logging a warning naming the cycle point.
""".
-spec flatten_ancestor_map(class_name(), fun((class_name()) -> #{atom() => term()})) ->
    #{atom() => term()}.
flatten_ancestor_map(ClassAtom, ReadOwnMapFun) ->
    case lookup_superclass(ClassAtom) of
        {ok, none} -> #{};
...
merge_ancestor_map(StartSuper, ReadOwnMapFun, AccSoFar) ->
    StepFun = fun({CurrentClass, Acc}, _Depth) ->
        ...
        NewAcc = maps:merge(OwnMap, Acc),
        case lookup_superclass(CurrentClass) of
    ...
    case beamtalk_hierarchy:walk_ancestors({StartSuper, AccSoFar}, StepFun, ?MAX_HIERARCHY_DEPTH) of
        ...
        {max_depth_exceeded, {CycleClass, Partial}} ->
            ?LOG_WARNING(
                "flatten_ancestor_map: max hierarchy depth ~p exceeded at ~p — possible cycle",
        ...
        not_found ->
            ...
            %% for a `none` StartNode, and the only caller here
            %% (flatten_ancestor_map/2) never invokes merge_ancestor_map/3
...
An indexed `ets:lookup/2` on the `beamtalk_class_subclass_index`
...
Gives `beamtalk_class_registry:module_to_class_map/0` an ETS-only
...
`?BEAMTALK_GENERATED_BUILTIN_CLASSES` (from the generated
...
%%====================================================================
%% Runtime class-method funs (ADR 0084)
%%====================================================================
...
Callers must also flip the gate flag via `set_runtime_class_methods/2`
...
class returns `error` without ever reading the funs table (the hot-path
...
Pairs with `delete_class_method_funs/1` when purging a class's funs
table ahead of a reload — together they are the reload path's explicit,
self-documenting substitute for a full-row overwrite's implicit reset
(see `merge_identity/5`, which deliberately leaves the gate alone).
...
%% Reconcile the {Superclass, ClassName} reverse-edge index with a
