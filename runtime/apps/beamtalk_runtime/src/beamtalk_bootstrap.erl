...
Bootstrap no longer creates class processes for ProtoObject,
...
ADR 0032 Phase 0: Bootstrap registers the 'Class' stub
...
ADR 0036 Phase 1: Bootstrap also registers the 'Metaclass' stub
...
ADR 0038 Phase 1: Bootstrap also registers the 'ClassBuilder' stub
...
ADR 0032 Phase 0: Also registers the 'Class' stub so the
class chain dispatch fallthrough is available from startup.

ADR 0036 Phase 1: Also registers the 'Metaclass' stub so that
metaclass objects dispatch through the Metaclass → Class chain.

ADR 0038 Phase 1: Also registers the 'ClassBuilder' stub so that
...
init(Parent) ->
    ...
    %% ADR 0032 Phase 0: Register the 'Class' stub.
    ...
    %% ADR 0036 Phase 1: Register the 'Metaclass' stub after 'Class'.
    %% Metaclass has superclass 'Class', so Class must be registered first.
    beamtalk_metaclass_bt:register_class(),

    %% ADR 0038 Phase 1: Register the 'ClassBuilder' stub after 'Metaclass'.
    ...
    %% ADR 0087 Phase 2: The Erlang stub classes above do not compile
