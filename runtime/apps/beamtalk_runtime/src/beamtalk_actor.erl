...
init(State) when is_map(State) ->
    ...
    case {maps:is_key('$beamtalk_class', State), maps:is_key('__methods__', State)} of
        {true, true} ->
            {ok, State};
        {false, _} ->
            {stop, {missing_key, '$beamtalk_class'}};
        {_, false} ->
...
make_self(State) ->
    #beamtalk_object{
        class = maps:get('$beamtalk_class', State),
    ...
    case Selector of
        ...
        instVarNames when Args =:= [] ->
            ...
            InstVarNames = [K || K <- AllKeys,
                            not lists:member(K, ['$beamtalk_class', '__class_mod__', '__methods__'])],
