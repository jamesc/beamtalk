...
from(Start) when is_integer(Start) ->
    make_stream(
        make_successor_gen(Start),
        iolist_to_binary([<<"Stream(from: ">>, integer_to_binary(Start), <<")">>])
    );
from(_) ->
...
from_by(Start, StepFun) when is_function(StepFun, 1) ->
    make_stream(
        make_step_gen(Start, StepFun),
        iolist_to_binary([<<"Stream(from: ">>, format_value(Start), <<" by: [...])">>])
    );
from_by(_, _) ->
...
on(Map) when is_map(Map) ->
    case beamtalk_tagged_map:class_of(Map) of
        ...
        undefined ->
            raise_type_error(
                'on:',
                <<"Dictionary is not streamable; use doWithKey: or keys/values">>
            );
        Other ->
            raise_type_error(
                'on:',
                iolist_to_binary([
                    <<"Expected a streamable collection, got ">>,
                    atom_to_binary(Other, utf8)
                ])
            )
    end;
on(_) ->
    raise_type_error('on:', <<"Expected a collection (List, String, or Set)">>).
