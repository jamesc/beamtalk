...
children_count_test() ->
    ...
    %% Should have 5 children: workspace_meta, beamtalk_repl, idle_monitor, actor_sup, session_sup
...
children_ids_test() ->
    ...
    ?assert(lists:member(beamtalk_repl, Ids)),
...
beamtalk_repl_spec_test() ->
    ...
    %% Find beamtalk_repl child spec
    [ReplSpec] = [S || S <- ChildSpecs, maps:get(id, S) == beamtalk_repl],
    
    %% Should be a worker, permanent
    ?assertEqual(worker, maps:get(type, ReplSpec)),
    ?assertEqual(permanent, maps:get(restart, ReplSpec)),
    
    %% Should start with port 49152 and bind to localhost (IP tuple format)
    ?assertEqual({beamtalk_repl, start_link, [49152, #{bind_ip => {127, 0, 0, 1}}]}, 
