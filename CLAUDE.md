...
- **No image mechanics:** Keep Smalltalk's vocabulary, drop mechanisms that assume one live image. In new code, never add REPL-injected names, bootstrap-set class-variable singletons, or anything whose meaning differs between REPL, `beamtalk run`, `beamtalk test` and releases. Stateless services are sealed class-side facades; `current` must ask the runtime at call time; context-bound services raise a structured error, never `nil`. The existing `BeamtalkInterface`/`WorkspaceInterface`/`TranscriptStream` singletons predate this rule and will be removed as ADR 0129 (Accepted) is implemented; don't copy their pattern. See `docs/beamtalk-principles.md` §4.
