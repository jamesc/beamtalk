...
- **No image mechanics:** Keep Smalltalk's vocabulary, drop mechanisms that assume one live image. Never add REPL-injected names, bootstrap-set class-variable singletons, or anything whose meaning differs between REPL, `beamtalk run`, `beamtalk test` and releases. Stateless services are sealed class-side facades; `current` must ask the runtime at call time; context-bound services raise a structured error, never `nil`. See `docs/beamtalk-principles.md` §4 and `docs/ADR/0129-class-side-system-facades.md`.
